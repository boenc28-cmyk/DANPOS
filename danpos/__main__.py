...
import argparse
from danpos.lib import (
    positionSelectorByGreatTSS,
    positionDicMinMax,
    positionDistance,
    positionSelectorByValue,
    positionSelectorByGeneStructure,
    batchOccAroundPoints,
    batchOccInRegions,
    plot,
    batchOccPSD,
    retrieve_positions_by_value,
    batchPositionDistanceDistribution,
    batchPositionValDistribution,
)
from danpos.wiq import wiq, wig2wiq
...
def runDANPOS(command=""):
    ...
    if command == "dpos":
        tname = "position"
    elif command == "dtriple":
        tname = "position, peak, and region"
    else:
        tname = command
    if (len(sys.argv) < 3) and ("-h" not in sys.argv) and ("--help" not in sys.argv):
        ...
        print(
            "\nusage:\n\npython danpos.py",
            command,
            "<path> [optional arguments]\n\nfor more help, please try: python danpos.py "
            + command
            + " -h\n",
        )
        return 0
    ...
    parser = argparse.ArgumentParser(
        ...
        description="",
        epilog="Kaifu Chen, et al. chenkaifu@gmail.com,  \
    ...
    parser.add_argument(
        "-H",
        "--exclude_high_percent",
        dest="exclude_high_percent",
        metavar="",
        default=0,
        type=float,
    ...
    if (
    ...
    ):  # print help information once required by user
        ...
        len(sys.argv) >= 3
    ):  # at least one parameter need to be specified: a path to input file or files
        try:
    ...
    )  # print the command line, this let the user to keep a log and remember what parameters they specified
    print(
        "\n", args
    )  # print all parameter values, this provide a eacy way for the user to double check the parameter values used by DANPOS

    if True:  # args.statis=='C' or args.statis=='P':
        temptestcut = args.testcut.split("-")
        if len(temptestcut) == 2:
            testcut = float(temptestcut[1]) - log10(float(temptestcut[0][:-1]))
        elif float(temptestcut[0]) > 0:
            testcut = 0 - log10(float(temptestcut[0]))
        else:
            testcut = 0

    if True:  # args.statis=='C' or args.statis=='P':
        temppheight = args.pheight.split("-")
        if len(temppheight) == 2:
            pheight = float(temppheight[1]) - log10(float(temppheight[0][:-1]))
        elif float(temppheight[0]) > 0:
            pheight = 0 - log10(float(temppheight[0]))
        else:
            pheight = 0
    nonzero = 0
    if args.nonzero == 0:
    ...
    if command == "dtriple":
        danpos(
            ...
            region_distance=args.region_dis,
            call_peak=args.call_peak,
            peak_width=args.peak_width,
            peak_distance=args.peak_dis,
            # position calling args
            call_position=args.call_pos,
            height=args.height,
            pheight=pheight,
            logp=testcut,
            width=args.width,
            ...
            edge=args.edge,
            fill_gap=args.gapfill,
            ratio=args.ratio,
            # occupancy processing args
            ref_region=args.position_reference,
            ...
            exclude_low_percent=args.exclude_low_percent,
            exclude_high_percent=args.exclude_high_percent,
            region_file=args.nor_region_file,
            both=True,
...
def wigProfileToGene(args, wgs, pos_neg="", outname="", outmode="w"):
    ...
    if args.genefile_aliases == None:
        gfnames = args.genefile_paths.split(",")
    else:
        gfnames = args.genefile_aliases.split(",")
        ...
        if len(list(wgs.keys())) >= 1:
            ...
            for i in range(len(gfiles)):
                ...
                if i == 0:
                    ...
                    outmode = "a"
                dic = batchOccAroundPoints(
                    wgs,
                    outname=outname + "_TSS",
                    groupname=gfname + pos_neg + ".tss",
                    outmode=outmode,
                    chrColID=1,
                    nameColID=0,
                    posColIDpos=3,
                    ...
                    flankup=args.flank_up,
                    flankdn=args.flank_dn,
                    vcal=args.vcal,
                    excludeP=args.excludeP,
                )
                if len(colors) >= len(wigaliases):
                    rcode += plot(
                        dic=dic,
                        names=wigaliases,
                        outname="",
                        main=gfname + pos_neg + ".tss",
                        nrow=args.plot_row,
                        ncol=args.plot_column,
                        xmin=args.plot_xmin,
                        xmax=args.plot_xmax,
                        ymin=args.plot_ymin,
                        ymax=args.plot_ymax,
        ...
        if len(gfiles) > 1:
            ...
            for wfname in wigaliases:
                ...
                for i in poses:
                    ...
                    fo.write(oline+'\n')
                """
                if len(colors) >= len(gfnames):
                    rcode += plot(
                        dic=dic,
                        names=gfnames,
                        outname="",
                        main=wfname + pos_neg + ".tss",
                        nrow=args.plot_row,
                        ncol=args.plot_column,
                        xmin=args.plot_xmin,
                        xmax=args.plot_xmax,
    ...
    if "TTS" in sites:
        ...
        if len(list(wgs.keys())) >= 1:
            ...
            for i in range(len(gfiles)):
                ...
                dic = batchOccAroundPoints(
                    wgs,
                    outname=outname + "_TTS",
                    groupname=gfname + pos_neg + ".tts",
                    outmode=outmode,
                    chrColID=1,
                    nameColID=0,
                    posColIDpos=4,
    ...
    if "CSS" in sites:
        ...
        if len(list(wgs.keys())) >= 1:
            ...
            for i in range(len(gfiles)):
                ...
                dic = batchOccAroundPoints(
                    wgs,
                    outname=outname + "_CSS",
                    groupname=gfname + pos_neg + ".css",
                    outmode=outmode,
                    chrColID=1,
                    nameColID=0,
                    posColIDpos=5,
    ...
    if "CTS" in sites:
        ...
        if len(list(wgs.keys())) >= 1:
            ...
            for i in range(len(gfiles)):
                ...
                dic = batchOccAroundPoints(
                    wgs,
                    outname=outname + "_CTS",
                    groupname=gfname + pos_neg + ".cts",
                    outmode=outmode,
                    chrColID=1,
                    nameColID=0,
                    posColIDpos=6,
    ...
    if "ESS" in sites:
        ...
        if len(list(wgs.keys())) >= 1:
            ...
            for i in range(len(gfiles)):
                ...
                dic = batchOccAroundPoints(
                    wgs,
                    outname=outname + "_ESS",
                    groupname=gfname + pos_neg + ".ess",
                    outmode=outmode,
                    chrColID=1,
                    nameColID=0,
                    posColIDpos=8,
    ...
    if "ETS" in sites:
        ...
        if len(list(wgs.keys())) >= 1:
            ...
            for i in range(len(gfiles)):
                ...
                dic = batchOccAroundPoints(
                    wgs,
                    outname=outname + "_ETS",
                    groupname=gfname + pos_neg + ".ets",
                    outmode=outmode,
                    chrColID=1,
                    nameColID=0,
                    posColIDpos=9,
        ...
        if len(list(wgs.keys())) >= 1:
            ...
            for i in range(len(gfiles)):
                ...
                if i == 0:
                    ...
                    outmode = "a"
                dic = batchOccInRegions(
                    wgs,
                    outname=outname + "_gene",
                    groupname=gfname + pos_neg + ".gene",
                    outmode=outmode,
                    chrColID=1,
                    nameColID=0,
                    startColIDpos=3,
...
def wigProfileToBed(args, wgs, pos_neg="", outname="", outmode="w"):
    ...
    wigaliases = args.wigfile_aliases.split(",")
    colors = args.plot_colors.split(",")
    if args.bed3file_aliases == None:
        gfnames = args.bed3file_paths.split(",")
    else:
        gfnames = args.bed3file_aliases.split(",")
    sites = args.genomic_sites.split(",")
    if "center" in sites:
        ...
        if len(list(wgs.keys())) >= 1:
            ...
            for i in range(len(gfiles)):
                ...
                for j in range(lcount):
                    col = glines[j].split()
                    glines[j] = "\t".join(
                        [str(j), col[0], str(div((int(col[1]) + int(col[2])), 2)), "+"]
                    )
                    # print glines[j]
                if i == 0:
                    outmode = "w"
                ...
                dic = batchOccAroundPoints(
                    wgs,
                    outname=outname + "_center",
                    groupname=gfname + pos_neg,
                    outmode=outmode,
                    chrColID=1,
                    nameColID=0,
                    posColIDpos=2,
...
def profile(command="profile"):
    ...
    parser.add_argument(
        ...
        dest="genomic_sites",
        metavar="",
        default="TSS,TTS,CSS,CTS,ESS,ETS,gene,center,region",
    ...
    parser.add_argument(
        ...
        dest="plot_colors",
        metavar="",
        default="black,gray,red,blue,orange,purple,skyblue,cyan,green,blue4,darkgoldenrod",
    ...
    if args.wigfile_aliases == None:
        args.wigfile_aliases = args.wigfile_paths
    wgs = Wigs()
    ...
    if len(wigpaths) != len(wigaliases):
        ...
        for i in range(len(wigpaths)):
            ...
            if wg.step != args.bin_size:
                if not wg.changeStep(args.bin_size):
                    return
    ...
    if args.pos_neg == 0 or args.pos_neg == 3:
        ...
        if args.genefile_paths != None:
            rcode += wigProfileToGene(
                args, wgs, pos_neg="", outname=args.name, outmode=outmode
            )
            outmode = "a"
        if args.bed3file_paths != None:
            rcode += wigProfileToBed(
                args, wgs, pos_neg="", outname=args.name, outmode=outmode
            )
    ...
    if args.pos_neg == 1 or args.pos_neg == 2 or args.pos_neg == 3:
        ...
        pwgs = deepcopy(wgs)
        for k in list(pwgs.keys()):
            pwgs.get(k).rvNeg()
        outmode = "w"
        if args.pos_neg == 1 or args.pos_neg == 2:
            if args.genefile_paths != None:
                rcode += wigProfileToGene(
                    args, pwgs, pos_neg=".pos", outname=args.name, outmode=outmode
                )
                outmode = "a"
            if args.bed3file_paths != None:
                wigProfileToBed(
            ...
            if args.genefile_paths != None:
                rcode += wigProfileToGene(
                    args, pwgs, pos_neg=".pos", outname=args.name, outmode="a"
                )
            if args.bed3file_paths != None:
                rcode += wigProfileToBed(
                    args, pwgs, pos_neg=".pos", outname=args.name, outmode="a"
    ...
    if args.pos_neg == -1 or args.pos_neg == 2 or args.pos_neg == 3:
        ...
        if args.pos_neg == 2 or args.pos_neg == 3:
            if args.genefile_paths != None:
                rcode += wigProfileToGene(
                    args, nwgs, pos_neg=".neg", outname=args.name, outmode="a"
                )
            if args.bed3file_paths != None:
                rcode += wigProfileToBed(
                    args, nwgs, pos_neg=".neg", outname=args.name, outmode="a"
            ...
            if args.genefile_paths != None:
                rcode += wigProfileToGene(
                    args, nwgs, pos_neg=".neg", outname=args.name, outmode=outmode
                )
                outmode = "a"
            if args.bed3file_paths != None:
                rcode += wigProfileToBed(
...
def runPositionStatistics(command="runPositionStatistics"):
    ...
    if len(positionfiles) != len(positionaliases):
        ...
        print(
            "please note that aliases for different files would be separated by the comma ','\n"
        )
        return 0
    for i in range(len(positionfiles)):
        ...
        if "fuzziness_score" in title:
            occ[filealias] = retrieve_positions_by_value(
                in_file=positionfile,
                out_file=None,
                cr_col_name="chr",
                pos_col_name="smt_pos",
                val_col_name="smt_value",
                direction_by=[],
                top_value=None,
                bottom_value=None,
            )
            dis[filealias] = positionDistance(occ[filealias])
            fuz[filealias] = retrieve_positions_by_value(
                in_file=positionfile,
                out_file=None,
                cr_col_name="chr",
                pos_col_name="smt_pos",
                val_col_name="fuzziness_score",
                direction_by=[],
                top_value=None,
                bottom_value=None,
            )
        elif "width_above_cutoff" in title:
            width[filealias] = retrieve_positions_by_value(
                in_file=positionfile,
                out_file=None,
                cr_col_name="chr",
                pos_col_name="center",
                val_col_name="width_above_cutoff",
                direction_by=[],
                top_value=None,
                bottom_value=None,
                log10trans=True,
            )
            auc[filealias] = retrieve_positions_by_value(
                in_file=positionfile,
                out_file=None,
                cr_col_name="chr",
                pos_col_name="center",
                val_col_name="total_signal",
                direction_by=[],
                top_value=None,
                bottom_value=None,
                log10trans=True,
            )
            height[filealias] = retrieve_positions_by_value(
                in_file=positionfile,
                out_file=None,
                cr_col_name="chr",
                pos_col_name="center",
    ...
    if len(occ) > 0:
        ...
        if args.dis_min == None:
            args.dis_min = 100
        if args.dis_max == None:
            args.dis_max = 250
        if args.dis_step == None:
            args.dis_step = 10

        disdic = batchPositionDistanceDistribution(
            occ,
            outname=args.name + ".distance",
            min=args.dis_min,
            max=args.dis_max,
            step=args.dis_step,
        )
    ...
    if len(occ) > 0:
        ...
        if args.occ_max == None or args.occ_min == None:
            if args.occ_min == None:
                args.occ_min = minmax[0]
            if args.occ_max == None:
                args.occ_max = minmax[1]
            if args.occ_step == None:
                args.occ_step = (args.occ_max - args.occ_min) / 100.0
        occdic = batchPositionValDistribution(
            occ,
            outname=args.name + ".occupancy",
            min=args.occ_min,
            max=args.occ_max,
            step=args.occ_step,
        )
    ...
    if len(fuz) > 0:
        ...
        if args.fuz_max == None or args.fuz_min == None:
            if args.fuz_min == None:
                args.fuz_min = minmax[0]
            if args.fuz_max == None:
                args.fuz_max = minmax[1]
            if args.fuz_step == None:
                args.fuz_step = (args.fuz_max - args.fuz_min) / 100.0
        fuzdic = batchPositionValDistribution(
            fuz,
            outname=args.name + ".fuzziness",
            min=args.fuz_min,
            max=args.fuz_max,
            step=args.fuz_step,
        )
    ...
    if len(width) > 0:
        ...
        if args.width_max == None or args.width_min == None:
            if args.width_min == None:
                args.width_min = max(minmax[0], 1)
            if args.width_max == None:
                args.width_max = minmax[1]
            if args.width_step == None:
                args.width_step = (args.width_max - args.width_min) / 100.0
        widthdic = batchPositionValDistribution(
            width,
            outname=args.name + ".width",
            min=args.width_min,
            max=args.width_max,
            step=args.width_step,
        )
        rcode += plot(
            dic=widthdic,
            outname="",
            main="log10width distribution",
            nrow=args.plot_row,
            ncol=args.plot_column,
            xmin=args.width_min,
            xmax=args.width_max,
    ...
    if len(auc) > 0:
        ...
        if args.total_signal_max == None or args.total_signal_min == None:
            if args.total_signal_min == None:
                args.total_signal_min = max(minmax[0], 1)
            if args.total_signal_max == None:
                args.total_signal_max = minmax[1]
            if args.total_signal_step == None:
                args.total_signal_step = (
                    args.total_signal_max - args.total_signal_min
                ) / 100.0
        ...
        aucdic = batchPositionValDistribution(
            auc,
            outname=args.name + ".total_signal",
            min=args.total_signal_min,
            max=args.total_signal_max,
            step=args.total_signal_step,
        )
        rcode += plot(
            dic=aucdic,
            outname="",
            main="log10total_signal distribution",
            nrow=args.plot_row,
            ncol=args.plot_column,
            xmin=args.total_signal_min,
            xmax=args.total_signal_max,
            ymin=None,
            ymax=None,
            xlab="log10 total signal",
            ylab="Percent",
            colors=args.plot_colors.split(","),
        )
    ...
    if len(height) > 0:
        ...
        if args.height_max == None or args.height_min == None:
            if args.height_min == None:
                args.height_min = max(minmax[0], 1)
            if args.height_max == None:
                args.height_max = minmax[1]
            if args.height_step == None:
                args.height_step = (args.height_max - args.height_min) / 100.0
        heightdic = batchPositionValDistribution(
            width,
            outname=args.name + ".height",
            min=args.height_min,
            max=args.height_max,
            step=args.height_step,
        )
        rcode += plot(
            dic=heightdic,
            outname="",
            main="log10height distribution",
            nrow=args.plot_row,
            ncol=args.plot_column,
            xmin=args.height_min,
            xmax=args.height_max,
            ymin=None,
            ymax=None,
            xlab="log10 height",
            ylab="Percent",
...
def runPositionSelector(command="runPositionSelector"):
    ...
    if args.genicSelector != None and args.GREATSelector != None:
        print(
            "Wrong: please don't use genicSelector and GREATSelector at the same time!"
        )
        return False
    ...
    if args.valueSelector != None:
        retr = positionSelectorByValue(positionLines=retr, selection=args.valueSelector)
    valsel = max(len(retr) - 1, 0)
    print(valsel, "(", valsel * 100.0 / max(total, 1), "percent) selected by values,")
    if args.genicSelector != None:
        retr = positionSelectorByGeneStructure(
            positionLines=retr,
            selection=args.genicSelector,
        ...
        print(
            genesel,
            "(",
            genesel * 100.0 / max(total, 1),
            "percent) selected by genic structure and value, accounting for",
            genesel * 100.0 / max(valsel, 1),
            "percent of value selected.",
        )
    if args.GREATSelector != None:
        sel = args.GREATSelector[6:]
        retr = positionSelectorByGreatTSS(
            positionLines=retr, selection=sel, geneFile=args.gene_file, chrbinsize=None
        )
        genesel = max(len(retr) - 1, 0)
        print(
            genesel,
            "(",
            genesel * 100.0 / max(total, 1),
            "percent) selected by the GREAT selector, accounting for",
            genesel * 100.0 / max(valsel, 1),
            "percent of value selected.",
        )
    if len(retr) > 1:
        ...
        if args.position_out[-3:] == "bed":
            ...
            for line in retr[1:]:
                ...
                col = line.split()
                if col[1] == "-" and col[2] == "-":
                    col[1], col[2] = str(int(col[3]) - 74), str(int(col[3]) + 74)
                elif col[1] == "-":
                    col[1] = str(int(col[2]) - 74)
                elif col[2] == "-":
                    col[2] = str(int(col[1]) + 74)
        ...
        if args.gene_out != None:
            ...
            for line in retr[1:]:
                ...
                for t in tcol:
                    ...
                    for ttt in tttcol:
                        ttttcol = ttt.split("/")
                        if ttttcol[0] not in d:
                            d[ttttcol[0]] = {}
                        if ttcol[0] not in d[ttttcol[0]]:
                            d[ttttcol[0]][ttcol[0]] = []
                        d[ttttcol[0]][ttcol[0]].append("/".join(col[:4]))
            for gene in d:
                line = gene + "\t"
                for site in d[gene]:
                    line += site + ":" + ",".join(d[gene][site]) + "|"
                gfo.write(line[:-1] + "\n")
            gfo.close()
        print("")
    else:
...
def retrievePositionValuesAtRanks(command="retrievePositionValuesAtRanks"):
    ...
    for tsel in sels:
        ...
        for r in sel[1:]:
            r = int(r)
            if r > 0:
                print("\trank", r, ":", values[r - 1])
            else:
                print("\trank", r, ":", values[r])
    print("")
...
def main():
    if len(sys.argv) > 1:
        if sys.argv[1] == "dpos":
            runDANPOS(command="dpos")
        elif sys.argv[1] == "dpeak":
            runDANPOS(command="dpeak")
        elif sys.argv[1] == "dregion":
            runDANPOS(command="dregion")
        elif sys.argv[1] == "dtriple":
            runDANPOS(command="dtriple")
        elif sys.argv[1] == "stat":
            runPositionStatistics()
        elif sys.argv[1] == "profile":
            profile()
        elif sys.argv[1] == "selector":
            runPositionSelector()
        elif sys.argv[1] == "valuesAtRanks":
            retrievePositionValuesAtRanks()
        elif sys.argv[1] == "wiq":
            wiq()
        elif sys.argv[1] == "wig2wiq":
            wig2wiq()
        else:
            printHelp()
    else:
