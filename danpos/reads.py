...
class reads:
    def __init__(self, file="", rlen=0, step=10, paired=False, cut=1e-10, format="bed"):
        ...
        self.step = step
        if file != "" and format == "bed" and paired:
            self.loadBedPaired(file=file, step=self.step, cut=cut)
        elif file != "" and format == "bed":
            self.loadBed(file=file, step=self.step, cut=cut)
        elif file != "" and format == "bam" and paired:
            self.loadBamPaired(file=file, step=self.step, cut=cut)
        elif file != "" and format == "bam":
            self.loadBam(file=file, step=self.step, cut=cut)
        elif file != "" and format == "sam" and paired:
            self.loadSamPaired(file=file, step=self.step, cut=cut)
        elif file != "" and format == "sam":
            self.loadSam(file=file, step=self.step, cut=cut)
        elif file != "" and format == "bowtie" and paired:
            self.loadBowtiePaired(file=file, step=self.step, cut=cut)
        elif file != "" and format == "bowtie":
            self.loadBowtie(file=file, step=self.step, cut=cut)
    ...
    def clearEmptyEnd(self):
        ...
        for chm in self.data:
            ...
            for str in self.data[chm]:
                ...
                if size <= 0:
                    continue
                while self.data[chm][str][size - 1] == 0 and size > 0:
                    size -= 1  # self.data[chm].pop()
                if final_size < size:
                    final_size = size
            for str in self.data[chm]:
                self.data[chm][str].resize(final_size, refcheck=0)
    ...
    def fragSizeDis(self, minsize=10, maxsize=300):
        ...
        avg = (
            self.mean() * self.step
        )  ##### '*self.step' is added by Kaifu on Aug 1st, 2012 #####
        ...
        while (
            ...
            - (
                functions.div(
                    float(str(functions.ppois(cut, avg.item())).split()[-1]), log(10)
                )
            )
            < lgpcut
        ):
        ...
        for chm in self.data:
            ...
            for stra in list(
                ...
                tchr[stra] = functions.div(
                    ((tchr[stra] ** 2) ** 0.5 + tchr[stra]), 2
                )  # remove all neative values
    ...
    def loadBowtie(self, file="", step=10, cut=1e-10):
        ...
        for line in infile:
            ...
            if num % 1000000 == 0:
                print(num, "reads parsed")
            if stra == "+":
                mid = functions.div(start, step)
            elif stra == "-":
                mid = functions.div(end, step)
        ...
        for line in infile:
            try:
                ...
                if len(tnames) == 2:
                    name = (
                        tnames[0] + name[-1]
                    )  ########## add by kaifu on sep 5, 2012 ########## some time the reads name have two words seperated by a space (not '\t')
                end = start + len(col[4])
                if len(end1) < 1:
            ...
            if end1[3][:-1] == end2[3][:-1]:
                if end1[5] == "+" and end2[5] == "-":
                    ...
                    print(
                        "pair error --- reads from same strand:\n",
                        end1,
                        "\n",
                        end2,
                        "\n",
                    ...
                    end1, end2 = [], []
                    continue
                if fragsize not in fragsizes:
                    fragsizes[fragsize] = 1
                else:
                    fragsizes[fragsize] += 1
            else:
    ...
    def loadBed(self, file="", step=10, cut=1e-10):  # ,loadcount=0):
        ...
        for line in infile:
            ...
            if num % 1000000 == 0:
                print(num, "reads parsed")
            if stra == "+":
                mid = int(functions.div(start, step))
            elif stra == "-":
                mid = int(functions.div(end, step))
            if chm not in self.data:
                self.data[chm] = {"+": numpy.array([0.0]), "-": numpy.array([0.0])}
                sizes[chm] = 0
            if mid >= sizes[chm]:
        ...
        for line in infile:
            ...
            if end1[3][:-1] == end2[3][:-1]:
                if end1[5] == "+" and end2[5] == "-":
                    chm, mid, fragsize = (
                        end1[0],
                        int(functions.div((end1[1] + end2[2]), (2 * step))),
                        end2[2] - end1[1],
                    )
                elif end1[5] == "-" and end2[5] == "+":
                    chm, mid, fragsize = (
                        end1[0],
                        int(functions.div((end1[2] + end2[1]), (2 * step))),
                        end2[1] - end1[2],
                    )
                else:
    ...
    def loadSamPaired(self, file="", step=10, cut=1e-10):
        ...
        for sam in sam_reader.fetch():
            try:
                ...
                mid = int(
                    np.floor(
                        functions.div((ref_start + next_ref_start + rlen), (2 * step))
                    )
                )
                if sam.is_reverse:
                    stra = "-"
    ...
    def toWig(self, fs=None, extend=0, mifrsz=10, mafrsz=300):
        ...
        step = self.step
        if fs == None:
            fs = self.fragSizeDis(minsize=mifrsz, maxsize=mafrsz)
        if extend <= 0:
            extend = fs
        print("extend to", extend)
        old_extend = extend
        fragsize, extend = (
            functions.div(fs, (2 * step)),
            functions.div(extend, (2 * step)),
        )
        wg = Wig(step=step)
        ...
        for chm in self.data:
            ...
            self.data[chm]["+"][fragsize:lth] = self.data[chm]["+"][
                0 : (lth - fragsize)
            ]
            for i in range(fragsize):
                self.data[chm]["+"][i] = 0
            self.data[chm]["+"][0 : (lth - fragsize)] += self.data[chm]["-"][
                fragsize:lth
            ]
        ...
        wg.foldChange(
            functions.div(old_extend * 1.0, wg.step)
        )  ##### added by Kaifu on May29, 2014
        return wg
