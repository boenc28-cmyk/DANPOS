...
class Wigs:
    ...
    def foldNormalize(self, scalepairs=None, sampling_total=None, nonzero=False):
        ...
        if sampling_total == None:
            ...
            for wig in names:
                ...
                if scalepairs == None:
                    wigs[wig].foldChange(functions.div(asum * 1.0, wsum[wig]))
                else:
                    wigs[wig].foldChange(
                        functions.div(scalepairs[wig] * 1.0, wsum[wig])
                    )
                sys.stdout.write(str(wigs[wig].sum()) + "\n")
        else:
            average_total = functions.div(
                sum(sampling_total.values()), len(list(sampling_total.keys()))
            )
            for name in names:
                sys.stdout.write(name + " from " + str(wigs[name].sum()) + " to ")
                if scalepairs == None:
                    wigs[name].foldChange(
                        functions.div(average_total * 1.0, sampling_total[name])
                    )
                else:
                    wigs[name].foldChange(
                        functions.div(scalepairs[name], sampling_total[name])
                    )
                sys.stdout.write(str(wigs[name].sum()) + "\n")
        if nonzero:
            sys.stdout.write(
                "further correction based on count of non-zero base pairs\n"
            )
            gsizes, non0sizes = {}, {}
            for wig in wigs:
    ...
    def samplingTotal(
        ...
        if (
            ...
            and exclude_high_percent == 0
            and region_file == None
        ):
            return None
        sampling_total = {}
        if region_file == None:
            sys.stdout.write(
                "calculate total signal in each sample after excluding the top "
                + str(exclude_high_percent)
                + " and bottom "
                + str(exclude_low_percent)
            ...
            if region_out_file != None:
                rg.save(region_out_file)
        else:
            sys.stdout.write(
                "calculate total signal in each sample in genomic regions defined by"
                + region_file
                + "\n"
            )
            rg = Wig(region_file)
        ...
        sys.stdout.write(
            str(rg.sum())
            + " ("
            + str(functions.div(rg.sum() * 100.0, rg.gsize()))
            + " %) of "
            + str(rg.gsize())
            + " base pairs calculated:\n"
        )
        for name in names:
            sys.stdout.write(
                name
                + str(sampling_total[name])
                + " ("
                + str(
                    functions.div(sampling_total[name] * 100.0, self.data[name].sum())
                )
                + "% of total)\n"
            )
    ...
    def nor(
        ...
        # return self.quantileNormalize()
            ...
            return self.foldNormalize(
                scalepairs=scalepairs, sampling_total=sampling_total, nonzero=nonzero
            )
        # elif nor=='S':
        ...
        # sys.stdout.write("\nNo normalization method appointed to be done here.\n")
            ...
            sys.stdout.write(
                "Normalization method " + str(nor) + " not applicable now\n"
            )
        return 0
    ...
    def ajClonal(self, cut=1e-10, extend=1):  ###### add by Kaifu on Nov 14, 2012
        ...
        if cut == "0":
            ...
            if float(cut) >= 1:
                ...
                co = cut.split("-")
                if len(co) == 2:
                    cut = float(co[1]) - log10(float(co[0][:-1]))
                else:
                    cut = 0 - log10(float(co[0]))
                lgpcut = cut
                cut = int(avg + 0.5)
                while (
                    0
                    - (
                        functions.div(
                            float(
                                str(
                                    functions.ppois(
                                        functions.div(cut * 1.0, extend),
                                        functions.div(avg * 1.0, extend),
                                    )
                                ).split()[-1]
                            ),
                            log(10),
        ...
        for chr in m.getChrs():
            ...
            tchrv = (
                m.data[chr] - tchrv + numpy.log(tchrv + 1)
            )  # the addition of '1' is to avoid log(0),"+numpy.log(tchrv+1)" is used to keep the rank order values
            sys.stdout.write(chr + ":")
            for k in ks:
                twg = self.get(k)
                if chr not in twg.data:
                    continue
                temp = twg.data[chr].sum()
                sys.stdout.write("\t" + str(k), "reduced from", temp, "to ")
                if chr not in twg.data:
                    twg.data[chr] = numpy.array([0.0])
                if twg.data[chr].size != tchrv.size:
                    twg.data[chr].resize(tchrv.size, refcheck=0)
                twg.data[chr] = functions.div(
                    tchrv * twg.data[chr], (m.data[chr] + 1e-100)
                )  # the addition of '1e-100' is to avoid devide by 0
                sys.stdout.write(
                    twg.data[chr].sum()
                    + ", percent removed: "
                    + str(100 - functions.div(twg.data[chr].sum() * 100.0, temp))
                    + "\n"
                )
        ...
        for k in wigs:
            ...
            for chr in wig.data:
                ...
                i, tnum, tsz = (
                    ...
                    int(
                        functions.div(
                            (functions.div(num * wig.chrSum(chr), oldsum)), wig.step
                        )
                    ),
                    tsz,
                )
