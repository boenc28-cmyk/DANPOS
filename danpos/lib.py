...
def batchOccInRegions(
    ...
    if step < 1:
        ...
        if len(steps) > 1:
            steps.sort()
            print(
                "step sizes in wiggles are not the same, will be set to a common step size",
                steps[0],
            )
            step = steps[0]
        else:
    ...
    for k in keys:
        ...
        if outname != None:
            tHeatMapName = outname + "_heatmap"
        else:
            tHeatMapName = "heatmap"
        if not os.path.isdir(tHeatMapName):
            os.mkdir(tHeatMapName)
        if groupname != None:
            tHeatMapName = os.path.join(
                tHeatMapName,
                os.path.split(groupname)[-1] + "." + os.path.split(k)[-1] + ".heatmap",
            )
        else:
            tHeatMapName = k + ".heatmap"
        dic[k] = occInRegions(
            wg=wg,
            chrColID=chrColID,
            nameColID=nameColID,
            startColIDpos=startColIDpos,
            startColIDneg=startColIDneg,
            endColIDpos=endColIDpos,
            endColIDneg=endColIDneg,
            straColID=straColID,
            step=step,
            sep=sep,
            second_sep=second_sep,
            lines=lines,
            heatmapname=tHeatMapName,
            avgcurvename=outname,
            flankup=flankup,
            flankdn=flankdn,
            vcal=vcal,
            excludeP=excludeP,
    ...
    if outname != None:
        ...
        poses.sort()
        if outmode != "w" and len(poses) != len(lines) - 1:
            return {}
...
def batchOccAroundPoints(
    ...
    for k in keys:
        ...
        if groupname != None:
            ...
            tHeatMapName = k + ".heatmap"
        dic[k] = occAroundPoints(
            wg=wg,
            chrColID=chrColID,
            nameColID=nameColID,
            posColIDpos=posColIDpos,
            posColIDneg=posColIDneg,
            straColID=straColID,
            step=step,
            sep=sep,
            second_sep=second_sep,
...
def batchOccPSD(wgs, outname=None):
    ...
    if len(steps) > 1:
        ...
        for k in keys:
            wgs.get(k).changeStep(step=steps[0])
    dic = {}
    for k in keys:
        dic[k] = occPSD(wgs.get(k), outname=None)
    if outname != None:
        fo = open(outname + ".xls", "w")
...
def batchPositionDistanceDistribution(
    data, outname=None, min=100, max=250, step=1
):  # ={'name':[]},outname=None,crColID=0,posColID=3,min=100,max=250,step=1):
    keys = list(data.keys())
    dic = {}
    for k in keys:
        dic[k] = positionDistanceDistribution(
            dic=data[k], min=min, max=max, step=step
        )  # lines=dic[k],outname=None,crColID=crColID,posColID=posColID,min=min,max=max,step=step)
    if outname != None:
        fo = open(outname + ".xls", "w")
...
def batchPositionValDistribution(data, outname=None, min=0, max=1500, step=3):
    ...
    dic = {}
    for k in keys:
        dic[k] = positionValDistribution(
            dic=data[k], outname=None, min=min, max=max, step=step
        )
    if outname != None:
        fo = open(outname + ".xls", "w")
...
def occAroundPoints(
    ...
    lst = {}
    for i in range(flankup + flankdn):
        lst[i] = []
    num = 0
    for line in lines:
        if line[:-1] == "\n":
        ...
        if chr not in wg.data:
            continue
        if stra == "+":
            if second_sep != None:
                poses = col[posColIDpos].split(second_sep)
            else:
                poses = [col[posColIDpos]]
            # print poses,line
        elif stra == "-":
            if second_sep != None:
                poses = col[posColIDneg].split(second_sep)
            else:
                poses = [col[posColIDneg]]
        for pos in poses:
            if pos == "":
                continue
            ...
            if stra == "+":
                for i in range(0 - flankup, flankdn):
                    try:
                        tlst[flankup + i] = wg.data[chr][tss + i]
                    except:
                        continue
            else:
            ...
            ol = (
                name
                + "\t"
                + "\t".join([str(regionmax), str(regionmin), str(regionsum)])
            )
            for i in range(flankup + flankdn):
                ol += "\t" + str(tlst[i])
            outf.write(ol + "\n")
...
def occInRegions(
    ...
    for i in range(0, flankdn):
        outf.write("\t+" + str(i + bin_count))
    outf.write("\n")
    if avgcurvename == None:
        avgcurvename = "avgcurvename"
    # outf2=open(avgcurvename+'.xls',"w")
    # outf2.write('pos\tvalue\n')
    ...
    for line in lines:
        ...
        if stra == "+":
            if second_sep != None:
                starts, ends = (
                    col[startColIDpos].split(second_sep),
                    col[endColIDpos].split(second_sep),
                )
            else:
                starts, ends = [col[startColIDpos]], [col[endColIDpos]]
        elif stra == "-":
            if second_sep != None:
                starts, ends = (
                    col[startColIDneg].split(second_sep),
                    col[endColIDneg].split(second_sep),
                )
            else:
                starts, ends = [col[startColIDneg]], [col[endColIDneg]]
        id, lth = 0, len(starts)
        if lth != len(ends):
            continue
        while id < lth:
            ...
            if starts[id] == "":
                continue
            tss, tes, otss, otes = (
                div(int(starts[id]), step),
                div(int(ends[id]), step),
                div(int(starts[id]), ostep),
                div(int(ends[id]), ostep),
            )
            if stra == "+":
                for i in range(0 - flankup, 0):
                    try:
                ...
                for i in range(0, bin_count):
                    try:
                        tlst[flankup + i] = owg.data[chr][otss + int(i * bstep)]
                    except:
                        continue
                for i in range(0, flankdn):
                    try:
                        tlst[flankup + bin_count + i] = wg.data[chr][tes + i]
                    except:
                        continue
            else:
                for i in range(0 - flankup, 0):
            ...
            for i in range(flankup + flankdn + bin_count):
                ol += "\t" + str(tlst[i])
            outf.write(ol + "\n")
...
def positionSelectorOld(
    positionLines=[],
    selection=None,
    geneFile=None,
    outGeneFile=None,
    chrbinsize=10000000000000,
):
    """
    ...
    if geneFile != None:
        ...
        for line in open(geneFile).readlines()[1:]:
            col = line.split()
            if col[2] == "+":
                gname, cr, stra, tss = col[0], col[1], col[2], int(col[3])
            else:
                gname, cr, stra, tss = col[0], col[1], col[2], int(col[4])
            if cr not in gd:
                gd[cr] = {}
    ...
    for line in positionLines[1:]:
        ...
        for sel in sels:
            if ok and sel[0] == "promoter":
                ...
                for i in [1, 2, 3]:
                    try:
                        poses.append(int(col[i]))
                    except:
                        continue
                minpos, maxpos = min(poses), max(poses)
                minbin, maxbin = (
                    int(div((minpos - flank), chrbinsize)),
                    int(div((maxpos + flank), chrbinsize)),
                )
                ...
                for bin in bins:
                    ...
                    for tss in gd[cr][bin]:
                        if gd[cr][bin][tss][0] == "+" and (
                            maxpos > tss + sel[1] and minpos < tss + sel[2]
                        ):
                            if outGeneFile != None:
                                if gd[cr][bin][tss][1] not in oglines:
                                    oglines[gd[cr][bin][tss][1]] = []
                                oglines[gd[cr][bin][tss][1]].append(
                                    gd[cr][bin][tss][1] + "\t" + line
                                )
                            genes += gd[cr][bin][tss][1] + ","
                        elif gd[cr][bin][tss][0] == "-" and (
                            maxpos > tss - sel[2] and minpos < tss - sel[1]
                        ):
                            if outGeneFile != None:
                                if gd[cr][bin][tss][1] not in oglines:
                                    oglines[gd[cr][bin][tss][1]] = []
...
def positionSelectorByGreatTSS(
    positionLines=[], selection="-5000:1000:1000000", geneFile=None, chrbinsize=None
):
    """
    positionLines:  The first line in positionLines must be the title line, each line should have a '\n' at the end, positionLines should be in the default output format of DANPOS, see DANPOS documentation for example
    """
    ...
    if len(sels) < 3:
        print(
            "Wrong! Less than three fields could be detected in the selector:",
            selection,
        )
        return []
    elif sels[0] == "":
        ...
        sels[0], sels[1] = int(sels[0]), int(sels[1])
    if sels[2] == "":
        sels[2] = 0
    else:
        sels[2] = int(sels[2])
    ...
    for cr in tgd:
        ...
        while i < lth:
            ...
            minpos, maxpos = (
                min(bstart, max(prebend, pos - sels[2])),
                max(bend, min(nxtbstart, pos + sels[2])),
            )
            for bin in range(div(minpos, chrbinsize), div(maxpos, chrbinsize) + 1):
                if bin not in gd[cr]["TSS"]:
                    gd[cr]["TSS"][bin] = {}
                if minpos not in gd[cr]["TSS"][bin]:
                    gd[cr]["TSS"][bin][minpos] = []
        ...
        for bin in bins:
            ...
            for start in gd[cr]["TSS"][bin]:
                for i in range(len(gd[cr]["TSS"][bin][start])):
                    gname, end, pos = (
                        gd[cr]["TSS"][bin][start][i][1],
                        gd[cr]["TSS"][bin][start][i][2],
                        gd[cr]["TSS"][bin][start][i][3],
                    )
                    if maxpos > start and minpos < end:
                        tgenes[gname + "/" + str(pos) + "/" + str(pos)] = 1
        if len(tgenes) != 0:
            ok += 1
...
def positionSelectorByGeneStructure(
    positionLines=[], selection=None, geneFile=None, chrbinsize=10000000000000
):
    """
    positionLines:  The first line in positionLines must be the title line, each line should have a '\n' at the end, positionLines should be in the default output format of DANPOS, see DANPOS documentation for example
    selection:  promoter:-350:50&control_smt_val:0:1000&0-log10(point_diff_pval):0:1e-10
    ...
    for line in open(geneFile).readlines()[1:]:
        ...
        if "TSS" in selsdic:
            if "TSS" not in gd[cr]:
                gd[cr]["TSS"] = {}
            if stra == "+":
                gname, cr, stra, pos = col[0], cr, stra, int(col[3])
            else:
                gname, cr, stra, pos = col[0], cr, stra, int(col[4])
            bin = int(div(pos, chrbinsize))
            if bin not in gd[cr]["TSS"]:
                gd[cr]["TSS"][bin] = {}
            if pos not in gd[cr]["TSS"][bin]:
                gd[cr]["TSS"][bin][pos] = []
        ...
        if "TTS" in selsdic:
            ...
            if stra == "+":
                ...
                gname, cr, stra, pos = col[0], cr, stra, int(col[3])
            bin = int(div(pos, chrbinsize))
            if bin not in gd[cr]["TTS"]:
                gd[cr]["TTS"][bin] = {}
            if pos not in gd[cr]["TTS"][bin]:
                gd[cr]["TTS"][bin][pos] = []
        ...
        if "CSS" in selsdic:
            if "CSS" not in gd[cr]:
                gd[cr]["CSS"] = {}
            if stra == "+":
                gname, cr, stra, pos = col[0], cr, stra, int(col[5])
            else:
                gname, cr, stra, pos = col[0], cr, stra, int(col[6])
            bin = int(div(pos, chrbinsize))
            if bin not in gd[cr]["CSS"]:
                gd[cr]["CSS"][bin] = {}
            if pos not in gd[cr]["CSS"][bin]:
                gd[cr]["CSS"][bin][pos] = []
        ...
        if "CTS" in selsdic:
            ...
            if stra == "+":
                ...
                gname, cr, stra, pos = col[0], cr, stra, int(col[5])
            bin = int(div(pos, chrbinsize))
            if bin not in gd[cr]["CTS"]:
                gd[cr]["CTS"][bin] = {}
            if pos not in gd[cr]["CTS"][bin]:
                gd[cr]["CTS"][bin][pos] = []
        ...
        if "ESS" in selsdic:
            if "ESS" not in gd[cr]:
                gd[cr]["ESS"] = {}
            if stra == "+":
                gname, cr, stra, poss = col[0], cr, stra, col[8][:-1].split(",")
            else:
                gname, cr, stra, poss = col[0], cr, stra, col[9][:-1].split(",")
            for pos in poss:
                pos = int(pos)
                bin = int(div(pos, chrbinsize))
                if bin not in gd[cr]["ESS"]:
                    gd[cr]["ESS"][bin] = {}
                if pos not in gd[cr]["ESS"][bin]:
                    gd[cr]["ESS"][bin][pos] = []
        ...
        if "ETS" in selsdic:
            ...
            for pos in poss:
                pos = int(pos)
                bin = int(div(pos, chrbinsize))
                if bin not in gd[cr]["ETS"]:
                    gd[cr]["ETS"][bin] = {}
                if pos not in gd[cr]["ETS"][bin]:
                    gd[cr]["ETS"][bin][pos] = []
                gd[cr]["ETS"][bin][pos].append([stra, gname, pos])
        if "exon" in selsdic:
            if "exon" not in gd[cr]:
                gd[cr]["exon"] = {}
            # if stra=='+':
            gname, cr, stra, starts, ends = (
                col[0],
                cr,
                stra,
                col[8][:-1].split(","),
            ...
            for i in range(num):
                ...
                for pos in range(start, end, chrbinsize):
                    bin = int(div(pos, chrbinsize))
                    if bin not in gd[cr]["exon"]:
                        gd[cr]["exon"][bin] = {}
                    if start not in gd[cr]["exon"][bin]:
                        gd[cr]["exon"][bin][start] = []
                    gd[cr]["exon"][bin][start].append([stra, gname, end])
        if "intron" in selsdic:
            if "intron" not in gd[cr]:
                gd[cr]["intron"] = {}
            # if stra=='+':
            gname, cr, stra, starts, ends = (
                col[0],
            ...
            for i in range(1, num):
                ...
                for pos in range(start, end, chrbinsize):
                    bin = int(div(pos, chrbinsize))
                    if bin not in gd[cr]["intron"]:
                        gd[cr]["intron"][bin] = {}
                    if start not in gd[cr]["intron"][bin]:
                        gd[cr]["intron"][bin][start] = []
                    gd[cr]["intron"][bin][start].append([stra, gname, end])
        if "gene" in selsdic:
            if "gene" not in gd[cr]:
                gd[cr]["gene"] = {}
            gname, cr, stra, start, end = col[0], cr, stra, int(col[3]), int(col[4])
            for pos in range(start, end, chrbinsize):
                bin = int(div(pos, chrbinsize))
                if bin not in gd[cr]["gene"]:
                    gd[cr]["gene"][bin] = {}
                if start not in gd[cr]["gene"][bin]:
                    gd[cr]["gene"][bin][start] = []
    ...
    for line in positionLines[1:]:
        ...
        for sel in sels:
            ...
            for bin in bins:
                if cr not in gd:
                    continue
                if bin not in gd[cr][sel[0]]:
                    continue
                for start in gd[cr][sel[0]][bin]:
                    for i in range(len(gd[cr][sel[0]][bin][start])):
                        end = gd[cr][sel[0]][bin][start][i][2]
                        if gd[cr][sel[0]][bin][start][i][0] == "+" and (
                            maxpos > start + sel[1] and minpos < end + sel[2]
                        ):
                            tgenes[
                                gd[cr][sel[0]][bin][start][i][1]
                                + "/"
                                + str(start)
                                + "/"
                                + str(end)
                            ] = 1
                        elif gd[cr][sel[0]][bin][start][i][0] == "-" and (
                            maxpos > start - sel[2] and minpos < end - sel[1]
                        ):
                            tgenes[
                                gd[cr][sel[0]][bin][start][i][1]
                                + "/"
...
def positionSelectorByValue(positionLines=[], selection=None):
    ...
    for line in positionLines[1:]:
        ...
        # print ok,len(sels)
        if andor == "and" and ok == len(sels):
            retr.append(line)  # all selection condition must be ok
        elif andor == "or" and ok > 0:
            retr.append(line)  # at least one selection condition must be ok
        # print andor, ok
    return retr


def retrieve_positions_by_value(
    in_file=None,
    out_file=None,
    cr_col_name="chr",
    pos_col_name="diff_smt_loca",
    val_col_name="point_diff_pval",
    direction_by=["treat_point_val", "control_point_val"],
    top_value=1e-7,
    bottom_value=0.0,
    log10trans=False,
):
    ...
    if len(direction_by) == 2:
        dirb = [ids[direction_by[0]], ids[direction_by[1]]]  ###################
    out = {}
    retrieve = 0
    if out_file:
        fo = open(out_file, "w")
    ...
    for line in lines[1:]:
        ...
        try:
            if log10trans:
                chr, pos, v = col[crid], int(col[posid]), log10(float(col[vid]) + 1)
            else:
                chr, pos, v = col[crid], int(col[posid]), float(col[vid])
        except Exception as e:
            print("An error occurred:", e)
            print(line)
            continue
        if top_value != None and v >= top_value:
            continue
        if bottom_value != None and v <= bottom_value:
            continue
        #################
        if len(direction_by) == 2:
            if v == 0:
...
def retrieve_positions_by_rank(
    in_file=None,
    out_file=None,
    cr_col_name="chr",
    pos_col_name="diff_smt_loca",
    ...
    retrieve = 0
    if toprank == None:
        toprank = 0  # len(aftersort)
    if bottomrank == None:
        bottomrank = len(aftersort)
    for i in range(toprank, bottomrank):
        ...
        ###################
        if out_file:
            fo.write(linesdic[aftersort[i][0]])
        if chr not in out:
            out[chr] = {}
        out[chr][int(pos)] = v
        retrieve += 1
...
def positions2Points(
    ...
    if neglog:
        ...
        for cr in summits:
            ...
            for pos in summits[cr]:
                if summits[cr][pos] > 0:
                    tsmit[cr][pos] = 0 - log10(summits[cr][pos])
                elif summits[cr][pos] < 0:
                    tsmit[cr][pos] = log10(0 - summits[cr][pos])
                else:
                    print(
                        "fail to do log transfer at",
                        cr,
                        pos,
                        "due to value",
                        summits[cr][pos],
    ...
    for chr in summits:
        ...
        for pos in summits[chr]:
            p = pos - (
                pos % step
            )  # note that the orginal summits located within step will be merged
            if p not in smts[chr]:
                smts[chr][p] = summits[chr][pos]
            elif rankby == "max":
                smts[chr][p] = max(smts[chr][p], summits[chr][pos])
            else:
                smts[chr][p] = min(smts[chr][p], summits[chr][pos])

    if out_file:
        fo = open(out_file, "w")
    if out_file:
        fo.write("name\tchr\tpos\tmax_value\tmin_value\tvalues\n")
    out = {}
    for line in lines:
        if line[:-1] == "\n":
            line = line[:-1]
        col = line.split(sep)
        if col[chrColID] not in smts:
            continue
        tup, tdn, pos, cr = (
            up - up % step,
            down - down % step,
        ...
        for pos in poses:
            ...
            if len(ps) > 0:
                ...
                for i in range(0, len(ps)):
                    tvs.append(str(vs[i]))
                line = "\t".join(
                    [col[nameColID], cr, str(pos), str(maxv), str(minv), ",".join(tvs)]
                )
                if rankby == "min":
                    out[line] = minv
                else:
    ...
    for chr in msmts:
        mcount += len(msmts[chr])
    for chr in smts:
        tcount += len(smts[chr])
    for chr in summits:
        ocount += len(summits[chr])
    ...
    if rankby == "min":
        aftersort = sorted(list(out.items()), key=itemgetter(1))
    else:
        aftersort = sorted(list(out.items()), key=itemgetter(1), reverse=True)
    rout = []
...
def plot(
    ...
    if names == None:
        names = list(dic.keys())
        names.sort()
    xmincal, xmaxcal, ymincal, ymaxcal = (
        min(dic[names[0]].keys()),
        max(dic[names[0]].keys()),
        min(dic[names[0]].values()),
        max(dic[names[0]].values()),
    )
    ...
    if ymax != None:
        ...
        ymaxcal += (ymaxcal - ymincal) * (len(names) * 0.12 + 0.1)
    ...
    if region_size == 0:
        ...
        # print at,lb
        rcode += (
            "axis(side=1,at=c(" + ",".join(at) + "),labels=c(" + ",".join(lb) + "))\n"
        )
...
def occPSD0(wg, outname=None):
    ...
    for j in range(int(div(lth * wg.step, 250)), int(div(lth * wg.step, 100) + 1)):
        ...
        if outname != None:
            fo.write(
                str(div(lth * wg.step * 1.0, j)) + "\t" + str(div(spe[j], wn)) + "\n"
            )
    return dic
...
def positionValDistribution(dic, outname=None, min=0, max=1500, step=3):
    ...
    for d in range(int(div(min, step)), int(div(max, step))):
        ...
        if outname != None:
            fo.write(str(d * step) + "\t" + str(vdic[d]) + "\n")
        odic[d * step] = vdic[d]
    rodic = {}
    for k in odic:
        if odic[k] > 0:
...
def positionAroundPoint(
    ...
    for line in lines:
        ...
        if second_sep == None:
            ...
            tsses = tss.split(second_sep)
        for tss in tsses:
            if tss == "":
                continue
            tss = int(tss)
            tss -= tss % step
            if chr not in smtdic:
                continue
            num += 1
            if stra == "+":
                ...
                for pos in range(tss - flankdn + step, tss + flankup + step, step):
                    if pos in smtdic[chr]:
                        if tss - pos in dis:
                            dis[tss - pos] += 1  # smtdic[chr][pos]
                        else:
                            dis[tss - pos] = 1  # smtdic[chr][pos]
    num *= step
    if outname != None:
        outf = open(outname + ".xls", "w")
    for k in range(0 - flankup, flankdn, step):
        if k not in dis:
            dis[k] = 0
        dis[k] = div(dis[k] * 1.0, num)
        if outname != None:
            outf.write(str(k) + "\t" + str(dis[k]) + "\n")
    return dis
...
def runall(
    ...
    for file in glob(os.path.join(pth, "*positions.xls")):
        fn = os.path.split(file)[-1]
        fn = re.sub("positions.xls", "", fn)
        smts[file] = retrieve_positions_by_rank(
            in_file=file,
            out_file=None,
            col_name="fuzziness_score",
            toprank=None,
            bottomrank=None,
            decreasing=False,
            direction_by=[],
        )
    dic = batchPositionValDistribution(
        data=smts,
        outname=os.path.join(outpath, "position_fuzziness_distribution"),
        min=minfuz,
        max=maxfuz,
        step=0.01,
    )
    ...
    dic = batchPositionValDistribution(
        data=smts,
        outname=os.path.join(outpath, "position_occupancy_distribution"),
        min=minocc,
        max=maxocc,
        step=3,
    )
    ...
    dic = batchPositionDistanceDistribution(
        data=smts,
        outname=os.path.join(outpath, "position_distance_distribution"),
        min=50,
        max=400,
        step=step,
    )
...
def positionDicMinMax(dic, lowPercent=0, highPercent=100):
    ...
    lth = len(values)
    outmin, outmax = (
        values[int(div(lth * lowPercent, 100.0))],
        values[int(div(lth * highPercent, 100.0)) - 1],
    )
    ...
    for name in dic:
        ...
        for chr in dic[name]:
            ...
            for value in list(dic[name][chr].values()):
                if value > outmin and value < outmax:
                    dvalues[name].append(value)
    return [outmin, outmax, dvalues]


def translocationReads(
    file,
    bindic={},
    binSize=1000000,
    outReadsFile="out.sam",
    outmode="a",
    pdis=3000,
    step=1,
    mapq=30,
    ...
    while line[0] == "@":
        ...
        if col[0] == "@SQ":
            ...
            if chr not in bindic:
                bindic[chr] = {}
            for bin in range(div(clen, binSize) + 1):
                if bin not in bindic[chr]:
                    bindic[chr][bin] = {}
    ...
    for line in infile:  # .readline()
        ...
        chr1, chr2, mapq1, mapq2, pos1, pos2 = (
            col1[2],
            col2[2],
            float(col1[4]),
            float(col2[4]),
        ...
        if mapq1 < mapq or mapq2 < mapq:
            nl, unique = nl + 2, False  # low mapping-quality ->non-unique
        tra, ter = False, False
        if chr1 == chr2:
            dis = pos1 - pos2
            if dis < 0:
        ...
        for line in [line1, line2]:
            ...
            if clipSize <= 0:
                ...
                if t2[0] == "S":
                    if int(t1[0]) >= clipSize:
                        tbre = True
                elif t2[-1] == "S":
                    if int(t1[-1]) >= clipSize:
                        tbre = True
            if tbre:
                bre = True  # set bre to True if tbre become True.
            mlth = 0
            ...
            for i in range(len(t2)):
                if (
                    t2[i] == "M"
                ):  # all matched nucleotide will be counted into wiggle format data.
                    end = start + t1[i]
                    allwig.data[col[2]][div(start, step) : div(end, step)] += div(
                        1.0, mlth
                    )
                    if unique and tbre and trans:
                        transwig.data[col[2]][div(start, step) : div(end, step)] += (
                            1.0  # /rdlth
                        )
                    start = end
                elif t2[i] == "D":
        ...
        if bre and trans:
            ...
            if ter and bre:
                ne += 2  # inter-chromosome translocation
            elif tra and bre:
                na += 2  # intra-chromosome translocation
        else:
            no += 2

    fo.close()
    if allWigFile != None:
        allwig.save(allWigFile)
    if transWigFile != None:
        transwig.save(transWigFile)

    print("all raw reads:", nt + 1)
    print("unmappable:", nu, div(nu * 100.0, nt), "%")
    print("low map quality (non-unique):", nl, div(nl * 100.0, nt), "%")
    if inter:
        print("inter-chromosome translocated and unique:", ne, div(ne * 100.0, nt), "%")
    if intra:
        print("intra-chromosome translocated and unique:", na, div(na * 100.0, nt), "%")
    print("other unique:", no, div(no * 100.0, nt), "%")
    print("All unique:", ne + na + no, div((ne + na + no) * 100.0, nt), "%")
    if "all" not in readsCount:
        readsCount["all"] = nt
    else:
        readsCount["all"] += nt
    if "unmappable" not in readsCount:
        readsCount["unmappable"] = nu
    else:
        readsCount["unmappable"] += nu
    if "non_unique" not in readsCount:
        readsCount["non_unique"] = nl
    else:
        readsCount["non_unique"] += nl
    if "unique_inter" not in readsCount:
        readsCount["unique_inter"] = ne
    else:
        readsCount["unique_inter"] += ne
    if "unique_intra" not in readsCount:
        readsCount["unique_intra"] = na
    else:
        readsCount["unique_intra"] += na
    if "unique_other" not in readsCount:
        readsCount["unique_other"] = no
    else:
        readsCount["unique_other"] += no
    if "unique" not in readsCount:
        readsCount["unique"] = ne + na + no
    else:
        readsCount["unique"] += ne + na + no
    if "mappable" not in readsCount:
        readsCount["mappable"] = ne + na + no + nl
    else:
        readsCount["mappable"] += ne + na + no + nl
    if "trans" not in readsCount:
        readsCount["trans"] = 0
    if inter:
        readsCount["trans"] += ne  # readsCount['unique_inter']
    if intra:
        readsCount["trans"] += na  # readsCount['unique_intra']
...
def translocationLinks(
    ...
    fisherTest = r(
        """function(ov,n1,n2,n){return(phyper(ov - 1, n1, n-n1, n2, lower.tail = FALSE))}"""
    )
    pks, lks = bindic, {}
    ...
    for cr in peaks:
        ...
        for start in starts:
            ...
            if wstep > 0:
                ...
                lks[id], bn[id] = {}, 0
                pklines.append(
                    "\t".join([cr, str(start), str(end), str(nstart), str(nend)])
                )
    ...
    for line in infile:
        ...
        for start in pks[cr1][bin1]:
            if pos1 > start and pos1 < pks[cr1][bin1][start][0]:
                id1.append(pks[cr1][bin1][start][1])
        for start in pks[cr2][bin2]:
            if pos2 > start and pos2 < pks[cr2][bin2][start][0]:
                id2.append(pks[cr2][bin2][start][1])
        if len(id2) == 0:
            id2 = [0]
        if len(id1) == 0:
            id1 = [0]
    ...
    print(
        "total edges:", tn, sum(bn.values())
    )  # tn may be smaller than the sum of values in bn or lks, due to the fact that there are overlapped bins in bn keys

    lkf = open(linkfile, "w")
    lkf.write(
        "chrA\tstartA\tendA\twindowAstart\twindowAend\tedgeA\tchrB\tstartB\tendB\twindowBstart\twindowBend\tedgeB\tedgeAB\texpected\tlog10P\n"
    )
    ids2 = deepcopy(ids)
    for fk in ids:
...
def mergeLinks(
    ...
    for line in pf:
        col = line.split()
        if float(col[fdColID]) < fold:
            continue
        if float(col[pvColID]) > logP:
            continue
    ...
    for cr in peaks:
        for start in peaks[cr]:
            fo.write(cr + "\t" + str(start) + "\t" + str(peaks[cr][start]) + "\n")
    fo.close()
    translocationLinks(
        peakFile=outlinkfile,
        samFile=samFile,
        linkfile=outlinkfile,
        binSize=binSize,
        wsize=0,
        wstep=0,
        fold=0,
...
def peaks2bed(pfile, bfile=None, flank=100):
    ...
    for line in pf:
        ...
        for tp in col[3].split(","):
            ps.append(int(tp))
        ps.sort()
        bf.write(
            "\t".join(
                [cr, str(max(ps[0] - flank, 0)), str(ps[-1] + flank), "1", "1", "+"]
            )
            + "\n"
        )
