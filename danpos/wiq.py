...
def qnorwig(
    ...
    for path in paths.split(":"):
        if os.path.isfile(path):
            files[path] = 1  # .append(path)
        elif os.path.isdir(path):
            if iformat == "wig":
                for file in glob(os.path.join(path, "*wig")):
                    files[file] = 1
            if iformat == "wiq":
                for file in glob(os.path.join(path, "*wiq")):
                    files[file] = 1
    for ifile in files:
        opath = re.sub("/", "_", ifile)  #'_'.join(elems)
        while opath[0] == "." or opath[0] == "_" or opath[0] == "/":
            opath = opath[1:]
...
if __name__ == "__main__":
    ...
    )  # This allows to print each message on screen immediately.
