...
def danpos(
    ...
    fcut = float(numpy.std(list(range(0 - rd, rd + step, step)))) * (
        0.95 - div(step, (2.0 * rd))
    )
    ...
    if save:
        if (
            ...
            or ((nor != "N") and ((len(groups) > 1) or (maxgroupsize > 1)))
        ):
            ...
            for groupname in groups:
                groups[groupname].save(
                    os.path.join(opath, "raw"), format=wgfmt, step=step
                )
    ...
    if tbg is not None:
        for bggroupname in bggroups:
            bggroups[bggroupname] = loadinput(
                bggroups[bggroupname],
                fs=100,
                cut=cut,
                save=False,
                wgfmt=wgfmt,
    ...
    if tbg is not None:
            ...
            if bggroupname != "None":
                if bggroupname not in pooledbggroups:
                    ...
                    if len(bgfilenames) > 1:
                        bggroups[bggroupname].nor(
                            nor="F",
                            exclude_low_percent=0,
                            exclude_high_percent=0,
                            scalepairs=None,
                            sampling_total=None,
                            nonzero=False,
                        )
                    pooledbggroups[bggroupname] = bggroups[bggroupname].pop(
                        bgfilenames[0]
                    )
                    if len(bgfilenames) > 1:
                        for bgfilename in bgfilenames[1:]:
                            pooledbggroups[bggroupname].add(
                                bggroups[bggroupname].pop(bgfilename)
                            )
                        pooledbggroups[bggroupname].foldChange(
                            div(1.0, len(bgfilenames))
                        )
                    if not os.path.isfile(
                        os.path.join(opath, "pooled", bggroupname + ".wig")
                    ):
                        pooledbggroups[bggroupname].save(
                            os.path.join(opath, "pooled", bggroupname + ".wig"),
                            format=wgfmt,
                            step=step,
                        )
        ...
        if save:
            if (
                ...
                for groupname in groups:
                    groups[groupname].save(
                        os.path.join(opath, "bgsub"), format=wgfmt, step=step
                    )
                print("time elapsed:", time() - starttime, "seconds")
        pooledbggroups = ""
        bggroups = ""
    ...
    if (nor != "N") and ((len(groups) > 1) or (maxgroupsize > 1) or amount != None):
        ...
        if amount == None:
            scalepairs = None
        sampling_total = wigs.samplingTotal(
    ...
    if (
        smooth_width > 1
    ):  # if not save, the smoothing will be done after pooling; else it will be done here
        addname += "smooth."
            ...
            for groupname in groups:
                for filename in list(groups[groupname].keys()):
                    ...
                    groups[groupname].set(
                        filename[:-3] + "smooth.wig", groups[groupname].pop(filename)
                    )
                if not os.path.isdir(os.path.join(opath, "smooth")):
                    os.mkdir(os.path.join(opath, "smooth"))
                groups[groupname].save(
                    os.path.join(opath, "smooth"), format=wgfmt, step=step
                )
            print("time elapsed:", time() - starttime, "seconds")
    # Step 6 completed.
    ...
    for groupname in groups:
        ...
        if (not save) and (smooth_width > 1):
            pooledgroups[groupname].smooth(
                smooth_width
            )  # if not save, the smoothing will be done here; else it have been done before replicate position calling
        if (
            (len(filenames) > 1)
            or (smooth_width > 1)
            or (cut != "0")
            ...
            pooledgroups[groupname].save(
                os.path.join(opath, "pooled", groupname + addname + "wig"),
                format=wgfmt,
                step=step,
            )
        print("time elapsed:", time() - starttime, "seconds")
    ...
    for i in range(len(pairs)):
        ...
        dfgroups[dfname] = pooledgroups[pairs[i][0]].dfTest(
            test=test, cwig=pooledgroups[pairs[i][1]]
        )
        # if len(dfgroups)>0:
        if not os.path.isdir(os.path.join(opath, "diff")):
            os.mkdir(os.path.join(opath, "diff"))
        for dfname in dfgroups:
            dfgroups[dfname].save(
                os.path.join(
                    opath, "diff", re.sub(":", "-", dfname + testname[test] + "wig")
                ),
                format=wgfmt,
                step=step,
            )
    ...
    if (call_region == 1) or (call_peak == 1):
        ...
        if len(dfgroups) > 0 and logp != 0:
            ...
            for dfname in dfgroups:
                ...
                dfpeakgroups[dfname] = dfgroups[dfname].callRegions(
                    ofile=None,
                    width=0,
                    distance=0,
                    pheight=1,
            ...
            for dfname in dfgroups:
                ...
                dfpeakgroups2[dfname] = dfgroups[dfname].callRegions(
                    ofile=None,
                    width=0,
                    distance=0,
                    pheight=1,
        ...
        if len(dfgroups) > 0 and logp != 0:
            peak_group_list += [dfpeakgroups, dfpeakgroups2]
        for (
            temp_peak_group
        ) in peak_group_list:  # merge positions from all group into regions
            for name in temp_peak_group:
                ...
                for chr in temp_peaks:
                    ...
                    for pos in temp_peaks[chr]:
                        if pos not in peaks[chr]:
                            peaks[chr][pos] = temp_peaks[chr][pos]
                        elif peaks[chr][pos] < temp_peaks[chr][pos]:
                            peaks[chr][pos] = temp_peaks[chr][pos]
    if call_peak == 1:
        if ref_peak != None:
            ...
            peaks = merge_peaks_by_head_tail_distance(
                peaks=peaks, distance=peak_distance
            )
            """
        ...
        for groupname in pooledgroups:
            print(groupname)
            temp_peaks = merge_peaks_by_head_tail_distance(
                peaks=peakgroups[groupname], distance=peak_distance
            )
            ...
            if len(pooledgroups) > 1:
                pooledgroups[groupname].fillRegions(
                    regions=peaks,
                    file=os.path.join(
                        opath, "pooled", groupname + addname + "refpeaks.xls"
                    ),
                    pheight=pheight,
                    height=height,
                    width=peak_width,
        ...
        if len(dfgroups) > 0:
            for dfname in dfgroups:
                ...
                if logp != 0:
                    temp_peaks = merge_peaks_by_head_tail_distance(
                        peaks=dfpeakgroups[dfname], distance=peak_distance
                    )
                if logp != 0:
                    dfgroups[dfname].fillRegions(
                        regions=temp_peaks,
                        file=re.sub(
                            ":",
                            "-",
                            os.path.join(
                                opath,
                                "diff",
                                dfname + testname[test] + "local_gain.peaks.xls",
                            ),
                        ),
                        pheight=pheight,
                        height=logp,
                ...
                dfgroups[dfname].fillRegions(
                    ...
                    file=re.sub(
                        ...
                        os.path.join(
                            opath,
                            "diff",
                            dfname + testname[test] + "local_gain.refpeaks.xls",
                        ),
                    ),
                    pheight=pheight,
                    height=logp,
                ...
                if logp != 0:
                    temp_peaks = merge_peaks_by_head_tail_distance(
                        peaks=dfpeakgroups2[dfname], distance=peak_distance
                    )
                if logp != 0:
                    dfgroups[dfname].fillRegions(
                        regions=temp_peaks,
                        file=re.sub(
                            ...
                            os.path.join(
                                opath,
                                "diff",
                                dfname + testname[test] + "local_loss.peaks.xls",
                            ),
                        ),
                        pheight=pheight,
                        height=logp,
                ...
                dfgroups[dfname].fillRegions(
                    ...
                    file=re.sub(
                        ...
                        os.path.join(
                            opath,
                            "diff",
                            dfname + testname[test] + "local_loss.refpeaks.xls",
                        ),
                    ),
                    pheight=pheight,
                    height=logp,
    ...
    if call_region == 1:
        if ref_region != None:
            ...
            print(
                "define reference regions by pooling regions defined in all sample..."
            )
            regions = merge_peaks_by_head_tail_distance(
                peaks=peaks, distance=region_distance
            )
            """
        ...
        for groupname in pooledgroups:
            print(groupname)
            temp_peaks = merge_peaks_by_head_tail_distance(
                peaks=peakgroups[groupname], distance=region_distance
            )
            pooledgroups[groupname].fillRegions(
                regions=temp_peaks,
                file=os.path.join(opath, "pooled", groupname + addname + "regions.xls"),
                pheight=pheight,
                height=height,
                width=region_width,
                calculate_P_value=1,
            ...
            if ref_region != None or len(pooledgroups) > 1:
                pooledgroups[groupname].fillRegions(
                    regions=regions,
                    file=os.path.join(
                        opath, "pooled", groupname + addname + "refregions.xls"
                    ),
                    pheight=pheight,
                    height=height,
                    width=region_width,
        ...
        if len(dfgroups) > 0:
            for dfname in dfgroups:
                ...
                if logp != 0:
                    temp_peaks = merge_peaks_by_head_tail_distance(
                        peaks=dfpeakgroups[dfname], distance=region_distance
                    )
                if logp != 0:
                    dfgroups[dfname].fillRegions(
                        regions=temp_peaks,
                        file=re.sub(
                            ...
                            os.path.join(
                                opath,
                                "diff",
                                dfname + testname[test] + "local_gain.regions.xls",
                            ),
                        ),
                        pheight=pheight,
                        height=logp,
                ...
                dfgroups[dfname].fillRegions(
                    ...
                    file=re.sub(
                        ...
                        os.path.join(
                            opath,
                            "diff",
                            dfname + testname[test] + "local_gain.refregions.xls",
                        ),
                    ),
                    pheight=pheight,
                    height=logp,
                ...
                if logp != 0:
                    temp_peaks = merge_peaks_by_head_tail_distance(
                        peaks=dfpeakgroups2[dfname], distance=region_distance
                    )
                if logp != 0:
                    dfgroups[dfname].fillRegions(
                        regions=regions,
                        file=re.sub(
                            ...
                            os.path.join(
                                opath,
                                "diff",
                                dfname + testname[test] + "local_loss.regions.xls",
                            ),
                        ),
                        pheight=pheight,
                        height=logp,
                ...
                dfgroups[dfname].fillRegions(
                    ...
                    file=re.sub(
                        ...
                        os.path.join(
                            opath,
                            "diff",
                            dfname + testname[test] + "local_loss.refregions.xls",
                        ),
                    ),
                    pheight=pheight,
                    height=logp,
    ...
    if call_position == 1:
        if call_region == 0 and call_peak == 0:
            peaks = None
        smtgroups = {}
        positionFiles = []
        for groupname in pooledgroups:
            print("\nposition calling for", groupname, "...")
            positionFiles.append(
                os.path.join(opath, "pooled", groupname + addname + "positions.xls")
            )
            smtgroups[groupname] = pooledgroups[groupname].callPositions(
                os.path.join(opath, "pooled", groupname + addname + "positions.xls"),
                width=width,
                distance=distance,
                edge=0,
        ...
        if ref_position != None:
            ...
            if len(pooledgroups) > 1:
                ...
                for chr in refdic:
                    ...
                    while i < lth:
                        fo.write(
                            chr + "\t" + str(refdic[chr]["p"][i]) + "\n"
                        )  # +str(refdic[chr]['v'][i])+'\n')
                        i += 1

        for groupname in pooledgroups:
            if len(pooledgroups) < 2:
                continue
            print(
                "\nfine-tuning positions for",
                groupname,
                "by comparing to the reference map...",
            )
            tdic = positionAdjust(
                dic=refdic,
                infile=os.path.join(
                    opath, "pooled", groupname + addname + "positions.xls"
                ),
                outfile=None,
                wg=pooledgroups[groupname],
                distance=distance,
                fcut=fcut,
                hdiff=hdiff,
            )
            pooledgroups[groupname].fillPositions(
                dic=tdic,
                file=os.path.join(
                    opath, "pooled", groupname + addname + "positions.ref_adjust.xls"
                ),
                width=width,
                distance=distance,
                edge=edge,
        ...
        if len(dfgroups) > 0:
            ...
            if logp != 0:
                ...
                for dfname in dfgroups:
                    ...
                    dsmtgroups[dfname] = dfgroups[dfname].callPositions(
                        re.sub(
                            ":",
                            "-",
                            os.path.join(
                                opath,
                                "diff",
                                dfname + testname[test] + "gain.positions.xls",
                            ),
                        ),
                        width=width,
                        distance=distance,
                ...
                for dfname in dfgroups:
                    ...
                    dfgroups[dfname].foldChange(-1)
                    dsmtgroups2[dfname] = dfgroups[dfname].callPositions(
                        re.sub(
                            ":",
                            "-",
                            os.path.join(
                                opath,
                                "diff",
                                dfname + testname[test] + "loss.positions.xls",
                            ),
                        ),
                        width=width,
                        distance=distance,
    ...
    if len(dfgroups) > 0:
        for dfname in dfgroups:
            ...
            tgs = 0
            for cr in dfgroups[dfname].data:
                tgs += dfgroups[dfname].data[cr].size
            ...
            if call_position == 1:
                ...
                fuzFDRlist = fuzFDR(
                    cwig=pooledgroups[groupnames[1]],
                    twig=pooledgroups[groupnames[0]],
                    simu=fdrsimu,
                    rd=rd,
                    regions=peaks,
                )
                ...
                print("analyzing...")
                if logp != 0:
                    gainPositionFile, lossPositionFile = (
                        re.sub(
                            ":",
                            "-",
                            os.path.join(
                    ...
                    gainPositionFile, lossPositionFile = None, None
                allPositionsInOneFile(
                    controlPositionFile=os.path.join(
                        opath,
                        "pooled",
                        groupnames[1] + addname + "positions.ref_adjust.xls",
                    ),
                    treatPositionFile=os.path.join(
                        opath,
                        "pooled",
                        groupnames[0] + addname + "positions.ref_adjust.xls",
                    ),
                    gainPositionFile=gainPositionFile,
                    lossPositionFile=lossPositionFile,
                    ofile=re.sub(
                        ":",
                        "-",
                        os.path.join(opath, dfname + ".positions.integrative.xls"),
                    ),
                    cwig=pooledgroups[groupnames[1]],
                    twig=pooledgroups[groupnames[0]],
                    dwig=dfgroups[dfname],
                    dis=distance,
                    rd=rd,
                    test=test,
                    fdrsimu=0,
                    fdrRegions=peaks,
                    occFDRlist=occFDRlist,
                    fuzFDRlist=fuzFDRlist,
                    fdr=fdr,
                )
                print("time elapsed:", time() - starttime, "seconds")
            if call_peak == 1:
                ...
                print("FDR simulation...")
                fdrlist = peakFDR(
                    peakFile1=os.path.join(
                        opath, "pooled", groupnames[1] + addname + "refpeaks.xls"
                    ),
                    wg1=pooledgroups[groupnames[1]],
                    wg2=pooledgroups[groupnames[0]],
                    fdrsimu=fdrsimu,
                    cut=height,
                )
                print("analyzing...")
                region_differential(
                    ...
                    file2=os.path.join(
                        opath, "pooled", groupnames[0] + addname + "refpeaks.xls"
                    ),
                    gainFile=re.sub(
                        ":",
                        "-",
                    ...
                    ofile=re.sub(
                        ":", "-", os.path.join(opath, dfname + ".peaks.integrative.xls")
                    ),
                    widthFDRlist=fdrlist[0],
                    smtFDRlist=fdrlist[1],
                    aucFDRlist=fdrlist[2],
                    step=step,
                    fdr=fdr,
                )
                print("time elapsed:", time() - starttime, "seconds")
            if call_region == 1:
                ...
                fdrlist = peakFDR(
                    peakFile1=os.path.join(
                        opath, "pooled", groupnames[1] + addname + "refregions.xls"
                    ),
                    wg1=pooledgroups[groupnames[1]],
                    wg2=pooledgroups[groupnames[0]],
                    fdrsimu=fdrsimu,
                ...
                region_differential(
                    ...
                    file2=os.path.join(
                        opath, "pooled", groupnames[0] + addname + "refregions.xls"
                    ),
                    gainFile=re.sub(
                        ":",
                        "-",
                    ...
                    ofile=re.sub(
                        ":",
                        "-",
                        os.path.join(opath, dfname + ".regions.integrative.xls"),
                    ),
                    widthFDRlist=fdrlist[0],
                    smtFDRlist=fdrlist[1],
                    aucFDRlist=fdrlist[2],
...
def pathParser(tpath):
    ...
    if tpath == None:
        ...
        for i in range(len(pairs)):
            ...
            if len(pairs[i]) > 2:
                print(
                    'Wrong: each pair can not contain more than 2 groups, please also make sure that each group name does not contain the symbol "-"'
                )
                return [{}, {}]
            for j in range(len(pairs[i])):
                if pairs[i][j] == "None":
                    continue
                if (not os.path.isfile(pairs[i][j])) and (
                    not os.path.isdir(pairs[i][j])
                ):
                ...
                while groupname[0] in [".", "_"]:
                    groupname = groupname[1:]
                if os.path.isfile(group):
                    if groupname[-6:] == "bowtie":
                ...
                if groupname in groups:
                    if groups[groupname] != group:
                        print(
                            "Wrong: different group (",
                            groups[groupname],
                            "and",
                            group,
                            ") were found for same group name (",
                            groupname,
                            ")",
                        )
                        return [{}, {}]
    ...
    if tbg != None:
        ...
        for i in range(len(bgpairs)):
            ...
            for j in range(len(bgpairs[i])):
                ...
                if bggroup == "None":
                    continue
                if (not os.path.isfile(bgpairs[i][j])) and (
                    not os.path.isdir(bgpairs[i][j])
                ):
                ...
                while bggroupname[0] in [".", "_"]:
                    bggroupname = bggroupname[1:]
                if os.path.isfile(bggroup):
                    if bggroupname[-6:] == "bowtie":
                        bggroupname = bggroupname[:-7]
                    else:
                        bggroupname = bggroupname[:-4]
                ...
                if (j == 1) and (bggroup != "None"):
                    if bggroupname in bggroups:
                        ...
                        bggroups[bggroupname] = bggroup
        # print bgpairs
        if (
            len(bgpairs) == 1 and len(bgpairs[0]) == 1
        ):  # all data set use the same background input
            if groups != None:
                bggroups[bgpairs[0][0]] = bggroup
                for name in groups:
                    subpairs[name] = bgpairs[0][0]
        else:
            for i in range(len(bgpairs)):
                if bgpairs[i][0] not in subpairs:
                    subpairs[bgpairs[i][0]] = bgpairs[i][1]
                elif subpairs[bgpairs[i][0]] != bgpairs[i][1]:
                    print(
                        "Wrong: different genomic background groups (",
                        subpairs[bgpairs[i][0]],
                        "and",
                        bgpairs[i][1],
                        ") were found for the same input group (",
                        bgpairs[i][0],
                        ")\n",
                    )
                    return [{}, {}]
...
def scaleParser(amount, extend, groups):
    ...
    if amount != None:
        ...
        for i in range(len(mpairs)):
            ...
            if mpairs[i][0] not in scalepairs:
                scalepairs[mpairs[i][0]] = mpairs[i][1] * extend
            elif scalepairs[mpairs[i][0]] != mpairs[i][1] * extend:
                print(
                    "Wrong: different expectation of amounts (",
                    scalepairs[mpairs[i][0]],
                    "and",
                    mpairs[i][1],
                    ") were found for the same group (",
                    mpairs[i][0],
                    ")\n",
                )
                return {}
        if len(scalepairs) != len(groups):
            print(
                "count of groups for scaling does not equal to count of input groups\n"
            )
            return {}
    return scalepairs


def region_differential(
    file1,
    file2,
    gainFile,
    lossFile,
    ...
    f1, f2, gf, lf = (
        ...
        open(gainFile).readlines(),
        open(lossFile).readlines(),
    )
    ...
    while i < lth:
        ...
        if w1 == w2:
            ...
            wdiff = log10PropTest([w1, w2, w2, w1])
        if t1 == t2:
            tdiff = 0
        else:
            tdiff = log10PropTest([t1, t2, t2, t1])
        if s1 == s2:
            sdiff = 0
        else:
            sdiff = float(
                (ppois(unnumpy(max(s1, s2) + 1), unnumpy(min(s1, s2) + 1)) / log(10))
            )
...
def peakFDR(peakFile1, peakFile2=None, wg1=None, wg2=None, fdrsimu=1000000, cut=5):
    ...
    for cr in wg1.data:
        if cr in wg2.data:
            chrs[cr] = min(wg1.data[cr].size, wg2.data[cr].size) - 1
    i, lth = 0, lth - 1
    w, s, t = (
        numpy.resize(numpy.array([0.0]), fdrsimu),
        numpy.resize(numpy.array([0.0]), fdrsimu),
        numpy.resize(numpy.array([0.0]), fdrsimu),
    )
    while i < fdrsimu:
        ...
        if end > chrs[cr] - width:
            start, end = chrs[cr] - width, chrs[cr]
        w1 = 0
        w2 = 0
        ...
        k = start
        while k < end:
            if wg1.data[cr][int(k)] >= cut:
                w1 += step
            if wg2.data[cr][int(k)] >= cut:
                w2 += step
            k += 1
        if w1 == w2:
            wdiff = 0
...
def merge_peaks_by_head_tail_distance(peaks, distance):
    ...
    for chr in regions:  # merge neighboring regions
        ...
        while i < lth:
            if (
                ps[i + 1] - regions[chr][ps[i]] <= region_distance
            ):  # or pks[chr][ps[i+1]]<=pks[chr][ps[i]]:
                if regions[chr][ps[i]] < regions[chr][ps[i + 1]]:
                    regions[chr][ps[i]] = regions[chr][ps[i + 1]]
        ...
        for path in tpath.split(":"):
            ...
            if os.path.isdir(path):
                for infile in glob.glob(os.path.join(path, "*.bed")):
                    return False
                for infile in glob.glob(os.path.join(path, "*.bowtie")):
                    return False
                ...
                for infile in glob.glob(os.path.join(path, "*.bed.gz")):
                    return False
                for infile in glob.glob(os.path.join(path, "*.bowtie.gz")):
                    return False
                for infile in glob.glob(os.path.join(path, "*.bam.gz")):
                    return False
                for infile in glob.glob(os.path.join(path, "*.sam.gz")):
                    return False
...
def loadinput(
    ...
    wigs = {}
    if os.path.isdir(path):
        for infile in glob.glob(os.path.join(path, "*.bed")) + glob.glob(
            os.path.join(path, "*.bed.gz")
        ):
            ...
            while fname[0] in [".", "_"]:
                fname = fname[1:]
            if paired:
                wigs[fname[:-4] + ".wig"] = rd.toWig(
                    fs=0, extend=extend, mifrsz=mifrsz, mafrsz=mafrsz
                )  # ,fdrwigs=fdrwigs,fdrflag=fdrflag)
            else:
                wigs[fname[:-4] + ".wig"] = rd.toWig(
                    fs=fs, extend=extend, mifrsz=mifrsz, mafrsz=mafrsz
                )  # ,fdrwigs=fdrwigs,fdrflag=fdrflag)
            rd = ""
            if save:
                wigs[fname[:-4] + ".wig"].save(
                    fname[:-4] + ".wig", format=wgfmt, step=step
                )
            if starttime != None:
                print("time elapsed:", time() - starttime, "seconds")
        for infile in glob.glob(os.path.join(path, "*.bowtie")) + glob.glob(
            os.path.join(path, "*.bowtie.gz")
        ):
            ...
            rd = ""
            if save:
                wigs[fname[:-7] + ".wig"].save(
                    fname[:-7] + ".wig", format=wgfmt, step=step
                )
            if starttime != None:
                print("time elapsed:", time() - starttime, "seconds")
        for infile in glob.glob(os.path.join(path, "*.bam")):
        ...
        for infile in glob.glob(os.path.join(path, "*.sam")):
            ...
            if starttime != None:
                print("time elapsed:", time() - starttime, "seconds")
        for infile in glob.glob(os.path.join(path, "*.wig")) + glob.glob(
            os.path.join(path, "*.wig.gz")
        ):
            fname = re.sub("/+", "_", infile)
            fname = re.sub(".gz$", "", fname)
            while fname[0] in [".", "_"]:
...
def combinePositions(
    controlPositionFile,
    treatPositionFile,
    gainPositionFile=None,
    lossPositionFile=None,
    dis=75,
):
    cpd = {}
    nc = 0
    ...
    for file in [gainPositionFile, lossPositionFile]:
        ...
        for line in open(file).readlines()[1:]:
            ...
            if abs(tdis) < dis or abs(cdis) < dis:
                if abs(tdis) < abs(cdis):
                    cpos = t2c[cr][pos + tdis]
                else:
                    cpos = pos + cdis
...
def allPositionsInOneFile(
    ofile="result.xls",
    controlPositionFile=None,
    treatPositionFile=None,
    gainPositionFile=None,
    lossPositionFile=None,
    cwig=None,
    twig=None,
    ...
    if cwig.step != twig.step:
        print(
            "step values are different in the control and treatment wiggle data, work can not be done"
        )
        return 0
    else:
        step = cwig.step
    print("combining nucleosome and differential positions...")
    c2tDic = combinePositions(
        controlPositionFile, treatPositionFile, gainPositionFile, lossPositionFile, dis
    )
    """
    cpd={}
    for line in open(controlPositionFile).readlines()[1:]:
    ...
    outf.write(
        "\t".join(
            [
                ...
                "treat_point_val",
                "point_log2FC",
                "point_diff_log10Pval",
                "point_diff_FDR",
                "control_fuzziness_score",
                "treat_fuzziness_score",
                "fuzziness_log2FC",
                "fuzziness_diff_log10pval",
                "fuzziness_diff_FDR\n",
            ]
        )
    )
    ...
    for cr in c2tDic:
        if cr in twig.data or cr in cwig.data:
            if cr not in twig.data:
                twig.data[cr] = deepcopy(cwig.data[cr]) * 0
            if cr not in cwig.data:
                cwig.data[cr] = deepcopy(twig.data[cr]) * 0
            if cr not in dwig.data:
                dwig.data[cr] = deepcopy(twig.data[cr]) * 0
        else:
            continue
        print(cr)
        ...
        for cpos in cposes:
            p1, p2 = (
                cpos,
                c2tDic[cr][cpos][0],
            )  # cpos is the control nucleosome position position, c2tDic[cr][cpos][0] is the treatment nucleosome position position
            if div(p1, step) >= cwig.data[cr].size:
                continue  # add by kaifu on Mar 10, 2013
            if div(p2, step) >= twig.data[cr].size:
                continue  # add by kaifu on Mar 10, 2013
            temp = log10fuztest(pc=p1, pt=p2, cr=cr, cwig=cwig, twig=twig, rd=rd)
            dp, sd1, sd2 = (
                temp  # ,float(r.sd(FloatVector(temp[1]))[0]),float(r.sd(FloatVector(temp[2]))[0])
            )
            nuc += 1
                ...
                if cwig.data[cr][div(p1, step)] > twig.data[cr][div(p2, step)]:
                    dp1 = float(
                        (
                            ...
                            - (
                                ppois(
                                    unnumpy(cwig.data[cr][div(p1, step)]),
                                    unnumpy(max(twig.data[cr][div(p2, step)], 1)),
                                )
                                / log(10)
                            )
                        )
                    ...
                    dp1 = float(
                        (
                            ...
                            - (
                                ppois(
                                    unnumpy(twig.data[cr][div(p2, step)]),
                                    unnumpy(max(cwig.data[cr][div(p1, step)], 1)),
                                )
                                / log(10)
                            )
                        )
            ...
            if (
                len(c2tDic[cr][cpos]) < 2
            ):  # no differential position is assigned to c2tDic[cr][cpos] (cpos is the control nucleosome position position, c2tDic[cr][cpos][0] is the treatment nucleosome position position)
                minp, maxp, maxv = (
                    min(
                        p1,
                        p2,
                        dwig.data[cr].size * step - step,
                        cwig.data[cr].size * step - step,
                        twig.data[cr].size * step - step,
                    ),
                    max(p1, p2),
                    0,
                )  # '*step' is add by kaifu on Mar 6, 2013
                ...
                p = div(
                    (minp + maxp), (2 * step)
                )  # replace '2' by '2*step' by kaifu on Mar 6, 2013
            ...
            if len(c2tDic[cr][cpos][1:]) > 2:
                for tp in c2tDic[cr][cpos][1:]:
                    tdp2 = abs(
                        dwig.data[cr][div(tp, step)]
                    )  # corrected by Kaifu Chen on Feb 13,2013, previously it was: tdp2=dwig.data[cr][tp/step]
                    if tdp2 > dp2:
                        p = tp
                        dp2 = tdp2
            dr2.append(dp2)
            ...
            if cr not in cpd:
                strp1 = "-"
            elif div(p1, step) not in cpd[cr]:
                strp1 = "-"
            else:
                strp1 = str(p1)
            if cr not in tpd:
                strp2 = "-"
            elif div(p2, step) not in tpd[cr]:
                strp2 = "-"
            else:
                strp2 = str(p2)
            if strp1 == "-" and strp2 == "-":
                strp1, strp2 = strp, strp
            elif strp1 == "-":
                strp1 == strp2
            elif strp2 == "-":
                strp2 = strp1
                ...
                [
                    ...
                    str(
                        div(
                            log(
                                div(
                                    max(twig.data[cr][div(p2, step)], 1),
                                    max(cwig.data[cr][div(p2, step)], 1),
                                )
                            ),
                            log(2),
                        )
                    ...
                    str(
                        div(
                            log(
                                div(
                                    max(1, twig.data[cr][div(p, step)]),
                                    max(cwig.data[cr][div(p, step)], 1),
                                )
                            ),
                            log(2),
                        )
    ...
    if fdr == 1:
        ...
        for i in range(len(dr1)):
            dr1[i] = div(findRank(occFDRlist, dr1[i]) * 1.0, fdrsimu)
        for i in range(len(dr2)):
            dr2[i] = div(findRank(occFDRlist, dr2[i]) * 1.0, fdrsimu)

        print("calculating fuzziness differential FDR...")
        if type(fuzFDRlist).__module__ == "builtins" and fuzFDRlist == None:
            fuzFDRlist = fuzFDR(
                cwig=cwig, twig=twig, simu=fdrsimu, rd=rd, regions=fdrRegions
            )
        for i in range(len(dr)):
            dr[i] = div(findRank(fuzFDRlist, 0 - dr[i]) * 1.0, fdrsimu)

    for line in olines:
        if fdr == 1:
            line[12], line[17], line[-2] = (
                str(dr1[line[-1]]),
                str(dr2[line[-1]]),
                str(dr[line[-1]]),
            )
...
def fuzFDR(cwig, twig, simu=10000, rd=None, regions=None):
    ...
    for cr in regions:
        for start in regions[cr]:
            ...
            if rstart < rd + cwig.step:
                rstart = rd + cwig.step
            if cr not in cwig.data:
                continue
            if cr not in twig.data:
                continue
            if rend > cwig.data[cr].size * cwig.step - rd - cwig.step:
                rend = cwig.data[cr].size * cwig.step - rd - cwig.step
            if rend > twig.data[cr].size * twig.step - rd - twig.step:
                rend = twig.data[cr].size * twig.step - rd - twig.step
            if rend <= rstart:
                continue
            i = 0
            while i < count:
...
def log10fuztest(pc, pt, cr, cwig, twig=None, rd=None):
    ...
    for d in range(-rd, rd + step, step):
        bv += d * d
    bvc = div(bv, bc)

    if twig == None:
        if pc >= (cwig.data[cr].size - 1) * rd:
            return [0, sqrt(bvc)]
        if pc < rd:
            return [0, sqrt(bvc)]
        ...
        if v >= bvc:
            ...
            p = pf(unnumpy(div(v, bvc)), unnumpy(c), unnumpy(bc))
            return [p, sqrt(v)]
    else:
        if cr not in twig.data:
            return fuztest(pc=pc, pt=pt, cr=cr, cwig=cwig, twig=None, rd=rd)
        if cr not in cwig.data:
            temp = fuztest(pc=pc, pt=pt, cr=cr, cwig=twig, twig=None, rd=rd)
            reurn[temp[0], temp[2], temp[1]]
        if pc >= (cwig.data[cr].size - 1) * rd:
            return [0, sqrt(bvc), sqrt(bvc)]
        if pt >= (twig.data[cr].size - 1) * rd:
            return [0, sqrt(bvc), sqrt(bvc)]
        if pc < rd:
            return [0, sqrt(bvc), sqrt(bvc)]
        if pt < rd:
        ...
        (
            vc,
            cc,
        ) = var(p=pc, cr=cr, wig=cwig, step=step, rd=rd, bv=bv, bc=bc)
        vt, ct = var(p=pt, cr=cr, wig=twig, step=step, rd=rd, bv=bv, bc=bc)
        if vc < vt:
            p = pf(unnumpy(div(vc, vt)), unnumpy(cc), unnumpy(ct))
        else:
            p = pf(unnumpy(div(vt, vc)), unnumpy(ct), unnumpy(cc))
...
def occFDR(dwig, simu=1000000, regions=None):
    ...
    for cr in regions:
        for start in regions[cr]:
            ...
            if rstart < dwig.step:
                rstart = dwig.step
            if rend > dwig.data[cr].size * dwig.step - dwig.step:
                rend = dwig.data[cr].size * dwig.step - dwig.step
            if rend <= rstart:
                continue
            i = 0
            while i < count:
                p = randint(rstart, rend)
                try:
                    vec[id] = 0 - abs(dwig.data[cr][div(p, dwig.step)])
                except:
                    print("wrong", id, vec.size, p, dwig.data[cr].size)
                id += 1
                i += 1
    vec.sort()
    return 0 - vec
                ...
                if td >= distance:
                    ...
                    if td2 < td:
                        ...
                        if vs[i] < vs[i + 1]:
                            ...
                            vs[i + 1] = div(
                                (vs[i] + vs[i + 1]), 2
                            )  # wg.data[cr][pos/wg.step] ###### added by Kaifu Chen Jul 10,2012 ######
            ...
            if (ps[-1] - ps[-2]) >= distance:
                ...
                if vs[-2] < vs[-1]:
                    ...
                    nps[ni], nvs[ni] = (
                        div((ps[-2] + ps[-1]), 2),
                        div((vs[-2] + vs[-1]), 2),
                    )  # wg.data[cr][((ps[-2]+ps[-1])/2)/wg.step] ###### added by Kaifu Chen Jul 10,2012 ######
                else:
...
def positionAdjust(dic, infile, outfile, wg, distance=100, fcut=None, hdiff=0.1):
    ...
    for line in lines[1:]:
        ...
        if fuz <= fcut:
            ...
            mi = max(mi1, mi2)
            if div((h - mi), h) >= hdiff and div((h - mi), h) >= hdiff * 1.5:
                tdic[cr][pos] = (
                    line  # retuire occupancy to be hdiff fold lower at one linker relative to position center, and 1.5*hdiff lower at the other linker.
                )
    ...
    for cr in dic:
        ...
        for pos in dic[cr]["p"]:
            if pos not in tdic[cr]:
                ...
                if abs(d) >= distance:
                    tdic[cr][pos] = "\t".join([cr, "-", "-", str(pos), "-", "-"]) + "\n"
    for cr in tdic:
        ps = list(tdic[cr].keys())
        ps.sort()
        if outfile != None:
            for p in ps:
                for line in tdic[cr][p]:
                    fo.write(line)
        tdic[cr] = {}
    ...
    for name in pkg:
        ...
        for cr in tpks:
            ...
            for start in tpks[cr]:
                if start not in pks[cr]:
                    pks[cr][start] = tpks[cr][start]
                elif pks[cr][start] < tpks[cr][start]:
                    pks[cr][start] = tpks[cr][start]
    for name in wd:
        wd[name].fillRegions(
            regions=pks,
            file=name[:-3] + "peaks.xls",
            pheight=pheight,
            height=height,
            width=width,
            calculate_P_value=1,
...
def scn(
    ...
    for groupname in groups:
        ...
        if os.path.isfile(path):
            groups[groupname], allWigs[groupname] = translocationReads(
                path,
                bindic=bindic,
                binSize=binSize,
                outReadsFile=groupfilename + ".sam",
                outmode="a",
                pdis=pdis,
                step=step,
                mapq=mapq,
    ...
    uwigs.data = allWigs
    sampling_total = uwigs.samplingTotal(
        exclude_low_percent=exclude_low_percent,
        exclude_high_percent=exclude_high_percent,
        bnum=bnum,
    )
    ...
    for groupname in groups:
        ...
        print(
            "\nnormalize",
            groupname,
            "wiggle data of all all reads from",
            tsum,
            "to",
            uwigs.data[groupname].sum(),
        )
        if saveWig:
            uwigs.data[groupname].save(os.path.join(name, groupname) + ".all.nor.wig")
    ...
    for groupname in groups:
        ...
        if saveWig:
            groups[groupname].save(os.path.join(name, groupname) + ".trans.nor.wig")
        # The value of groups[groupname] will be setted as the peaks data.
        groups[groupname] = groups[groupname].callRegions(
            ofile=os.path.join(name, groupname) + ".trans.sites.xls",
            width=width,
            distance=distance,
            pheight=pheight,
            height=height,
    ...
    for tpks in groups:
        for cr in groups[tpks]:
            ...
            for start in groups[tpks][cr]:
                if start not in pks[cr]:
                    pks[cr][start] = groups[tpks][cr][start]
                elif pks[cr][start] < groups[tpks][cr][start]:
                    pks[cr][start] = groups[tpks][cr][start]
    ...
    for groupname in groups:
        ...
        translocationLinks(
            peaks=pks,
            samFile=os.path.join(name, groupname + ".sam"),
            linkfile=os.path.join(name, groupname + ".trans.links.xls"),
            bindic=bindic,
            fold=0,
            logP=0,
            binSize=binSize,
    ...
    if len(pairs) > 0:
        for pair in pairs:
            ...
            diffwig.callRegions(
                ofile=os.path.join(
                    name, pair[0] + "-" + pair[1] + ".all.cnv.sites.gain.xls"
                ),
                width=width,
                distance=distance,
                pheight=pheight,
            ...
            diffwig.callRegions(
                ofile=os.path.join(
                    name, pair[0] + "-" + pair[1] + ".all.cnv.sites.loss.xls"
                ),
                width=width,
                distance=distance,
                pheight=pheight,
            ...
            tn1, tn2, sf1, sf2, lf1, lf2, lf = (
                sampling_total[pair[0]],
                sampling_total[pair[1]],
                os.path.join(name, pair[0] + ".trans.sam"),
                os.path.join(name, pair[1] + ".trans.sam"),
                os.path.join(name, pair[0] + ".trans.links.xls"),
                os.path.join(name, pair[1] + ".trans.links.xls"),
                os.path.join(name, "-".join(pair) + ".trans.links.xls"),
            )
            scnCompare(
                tn1=tn1,
                tn2=tn2,
    ...
    f.write(
        "\t".join(
            ...
            + [
                ...
                "log10PA",
                "log10PB",
                "normalized_observationA",
                "normalized_observationB",
                "log2FC",
                "log10Pdiff",
            ]
        )
...
if __name__ == "__main__":
    ...
    )  # This allow DANPOS to print each message on screen immediately.
