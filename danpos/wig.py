...
class Wig:
    def __init__(self, file="", gfile="", step=0, suppress=False):
        ...
        if gfile != "":
            for line in open(gfile):
                ...
                if step > 0:
                    self.data[col[0]].resize(
                        functions.div(int(col[1]), step) + 1, refcheck=0
                    )
                else:
                    self.data[col[0]].resize(int(col[1]) + 1, refcheck=0)
        if file != "":
            self.load(file, gfile=gfile, suppress=suppress)
    ...
    def add(self, wig2):
        ...
        for chrom in self.data:
            chrs[chrom] = 1
        for chrom in wig2.data:
            chrs[chrom] = 1
        chrs = list(chrs.keys())
        for chrom in chrs:
            if chrom not in self.data:
                self.addChr(chrom)
            if chrom not in wig2.data:
                wig2.addChr(chrom)
            lth1 = self.data[chrom].size
            lth2 = wig2.data[chrom].size
            if lth1 > lth2:
                wig2.data[chrom].resize(lth1, refcheck=0)
            elif lth1 < lth2:
                self.data[chrom].resize(lth2, refcheck=0)
    ...
    def ajust_size(self, gfile):
        ...
        for line in open(gfile):
            ...
            crs[col[0]] = size
            if size < self.data[col[0]].size:
                self.data[col[0]] = self.data[col[0]][
                    :size
                ]  # resize(int(col[1])/step+1,refcheck=0)
            if size > self.data[col[0]].size:
                self.data[col[0]].resize(size, refcheck=0)
    ...
    def correlation(self, wg2, ofile, mind=0, maxd=50):
        ...
        for chrom in chrs:
            ...
            if chrom in chrs2:
                ...
                if self.chrSize(chrom) != cs:
                    self.resizeChr(chrom, cs)
                if wg2.chrSize(chrom) != cs:
                    wg2.resizeChr(chrom, cs)
                for td in range(int(mind), int(maxd + 1)):
                    v = r.cor(
                        FloatVector(
                            self.getChr(chrom)[: (functions.div(cs, self.step) - td)]
                        ),
                        FloatVector(
                            wg2.getChr(chrom)[td : functions.div(cs, self.step)]
                        ),
                    )
    ...
    def callRegions(
        ...
        if height == 0 and pheight != 0:
            ...
            height = int(m + 0.5)
            while (
                0 - functions.div(float(functions.ppois(height, m.item())), log(10))
            ) < pheight:
                height += 1
        # if not suppress:
        sys.stdout.write(
            "whole genome aveage value is "
            + str(m)
            + ", use calling cutoff "
            + str(height)
            + "\n"
        ...
        for chrom in dic:
            ...
            while pos < lth:
                if start < 0:
                    if dic[chrom][pos] >= height:
                        start = pos
                elif dic[chrom][pos] < height:
                    if pos - start >= twidth:
                        pks[chrom][start * step] = (pos) * step
                    start = -1
                pos += 1
            if not suppress:
                sys.stdout.write("%i\n" % len(pks[chrom]))
        if distance > 0:
            if not suppress:
                sys.stdout.write("mering\n")
            crs = list(pks.keys())
            crs.sort()
            for chrom in crs:
                ps = list(pks[chrom].keys())
                if not suppress:
                    sys.stdout.write(chrom + " from " + str(len(ps)) + " ")
                ps.sort()
                lth = len(ps) - 1
                i = 0
                while i < lth:
                    ...
                    if (
                        ps[i + 1] - pks[chrom][ps[i]] <= distance
                    ):  # or pks[chrom][ps[i+1]]<=pks[chrom][ps[i]]:
                        if pks[chrom][ps[i]] < pks[chrom][ps[i + 1]]:
                            pks[chrom][ps[i]] = pks[chrom][ps[i + 1]]
                        pks[chrom].pop(ps[i + 1])
                        ps[i + 1] = ps[i]
                    i += 1
                if not suppress:
                    sys.stdout.write("to " + str(len(pks[chrom])) + "\n")
        if ofile != None:
            ...
            if title_line:
                if mode == "w":
                    if calculate_P_value:
                        outf.write(
                            "chr\tstart\tend\tsummit_pos\tsummit_value\tstrand\ttotal_signal\twidth_above_cutoff\tsummit_minus_logP\n"
                        )
                    else:
                        outf.write(
                            "chr\tstart\tend\tsummit_pos\tsummit_value\tstrand\ttotal_signal\twidth_above_cutoff\n"
                        )
            crs = list(pks.keys())
            crs.sort()
            for chrom in crs:
                ...
                for p in ps:
                    ...
                    if (e - s) >= twidth:  # and (v>=sheight):
                        ...
                        for i in range(s, e):
                            if dic[chrom][i] == v:
                                smts.append(str(i * step))
                            if dic[chrom][i] >= height:
                                width_above_cutoff += step
                            if pos_only:
                                if dic[chrom][i] > 0:
                                    auc += dic[chrom][i]
                            else:
                        ...
                        if calculate_P_value:
                            pvl = functions.div(
                                float(functions.ppois(v.item(), m.item())), log(10)
                            )
                            outf.write(
                                chrom
                                + "\t"
    ...
    def fillRegions(
        ...
        if calculate_P_value == 1:
            outf.write(
                "chr\tstart\tend\tcenter\twidth_above_cutoff\ttotal_signal\theight\theight_logP\n"
            )
        else:
            outf.write(
                "chr\tstart\tend\tcenter\twidth_above_cutoff\ttotal_signal\theight\n"
            )
        step = self.step
        dic = self.data
        ...
        for chrom in regions:
            ...
            starts.sort()
            for start in starts:
                if regions[chrom][start] - start < width:
                    continue
                s, e = (
                    functions.div(start, step),
                    functions.div(regions[chrom][start], step) + 1,
                )
                if dic[chrom].size <= e:
                    dic[chrom].resize(e + 1, refcheck=0)
                width_above_cutoff = 0
                v = max(dic[chrom][s:e])
                if pos_only:
                    v = max(v, 0)
                ...
                if calculate_P_value:
                    ...
                    outf.write(
                        ...
                        + str(start)
                        + "\t"
                        + str(regions[chrom][start])
                        + "\t"
                        + str(functions.div((regions[chrom][start] + start), 2))
                        + "\t"
                        + str(width_above_cutoff)
                        + "\t"
                        + str(auc)
        ...
        sys.stdout.write(
            "total_width_above_cutoff: " + str(total_width_above_cutoff) + "\n"
        )
        outf.close()
        ...
        if mode == "w":
            if poscal > 0:
                if calculate_P_value:
                    ...
                    outf.write("chr\tstart\tend\tsmt_pos\tsmt_value\tfuzziness_score\n")
            else:
                if calculate_P_value:
                    outf.write("chr\tstart\tend\tsmt_pos\tsmt_value\tsmt_log10pval\n")
                else:
                    outf.write("chr\tstart\tend\tsmt_pos\tsmt_value\n")
        twig = self
        # ppois=r('''function(q,avg){return(ppois(q,avg,lower.tail=FALSE,log.p=TRUE)/log(10))}''')
        m = self.mean()
        if height == 0 and pcut != 0:
            height = int(m + 0.5)
            while (0 - float(functions.ppois(height, m.item()) / log(10))) < pcut:
                height += 1
        # if not suppress:
        sys.stdout.write(
            "whole genome aveage value is "
                ...
                if edge == 0:
                    ...
                    while start == 0:
                        ...
                        if ppp - p >= functions.div(halfdis, 2):
                            if twig.data[chrom][p] == min(
                                twig.data[chrom][(p - width) : (p + width + 1)]
                            ):
                                start = p * twig.step + 1
                            elif twig.data[chrom][p] > valus[i]:
                                start = p * twig.step + 1
                            elif twig.data[chrom][p] < height:
                                start = p * twig.step + 1
                        elif i > 0 and p * twig.step == poses[i - 1]:
                            start = p * twig.step + 1
                        p -= 1
                    p = ppp + 1
                    while end == 0:
                        ...
                        elif i < (tlen - 1) and p * twig.step == poses[i + 1]:
                            end = p * twig.step + 1
                        p += 1
                if end > start:
                    if poscal > 0:
    ...
    def callSummits(self, width=40, pcut=1, height=5, regions=None):
        ...
        if pcut != 1:
            qp = r(
                """function(p,avg){return(qpois(log(p),avg,lower.tail=FALSE,log=TRUE))}"""
            )
            m = self.mean()
    ...
    def changeStep(self, step):
        ...
        if step == self.step:
            return True
        sys.stdout.write(
            "change wiggle step from " + str(self.step) + " to " + str(step) + "\n"
        )
        if step > self.step:
            if step % self.step != 0:
                sys.stdout.write(
                    "Wrong: the fold change between new step "
                    + str(step)
                    + " and old step "
                    + str(self.step)
                    + " is not integer!"
            ...
            for chrom in self.data:
                ...
                for npos in range(0, nlth):
                    for pos in range(npos * fd, (npos + 1) * fd):
                        lst[npos] += self.data[chrom][pos]
                self.data[chrom] = lst
        ...
        if step < self.step:
            ...
            for chrom in self.data:
                ...
                for pos in range(0, lth):
                    for npos in range(pos * fd, (pos + 1) * fd):
                        lst[npos] = self.data[chrom][pos]
                self.data[chrom] = lst
    ...
    def chisqTest(self, wig2, tchr=""):
        ...
        for chrom in wig1.data:
            tlen += len(wig1.data[chrom])
        donenum = 0
        ctime = time()
        for chrom in wig1.data:
            if tchr != "" and chr != tchr:
                continue
            if chrom in wig2.data:
                ...
                lth = max(lth1, lth2)
                if lth > lth2:
                    wig2.data[chrom].resize(lth, refcheck=0)
                elif lth > lth1:
                    wig1.data[chrom].resize(lth, refcheck=0)
                ...
                for p in range(0, lth):
                    ...
                    vec = r.c(
                        wig1.data[chrom][p],
                        wig2.data[chrom][p],
                        sum1 - wig1.data[chrom][p],
                        sum2 - wig2.data[chrom][p],
                    )
                    ...
                    if time() - ctime > 10:
                        ctime = time()
                        sys.stdout.write(
                            str(functions.div(donenum * 100.0, tlen))
                            + " percent of "
                            + str(tlen)
                            + " done\n"
                        )
    ...
    def chrSize(self, chrom):
        ...
        if chrom in self.data:
            return self.data[chrom].size * self.step
        else:
            return 0
    ...
    def clearEmptyEnd(self):
        ...
        for chrom in self.data:
            ...
            while self.data[chrom][size - 1] == 0 and size > 0:
                size -= 1  # self.data[chrom].pop()
    ...
    def divideAndLog2(self, wig2):
        ...
        for chrom in chrs:
            ...
            if lth1 > lth2:
                ...
                wig1.data[chrom].resize(lth2, refcheck=0)
            lth = max(lth1, lth2)
            wig1.data[chrom] += 1
            wig2.data[chrom] += 1
    ...
    def dfTest(self, cwig, test="C"):
        ...
        if test == "C":
            ...
            sys.stdout.write(
                "No test method appointed, will not do any differential test.\n"
            )
            return Wig(step=self.step)
        else:
            sys.stdout.write(
                "Normalization method " + str(test) + " not applicable now\n"
            )
            return Wig(step=self.step)
    ...
    def fisherTest(self, wig2):
        ...
        for chrom in self.data:
            if chr in wig2.data:
                ...
                for p in range(0, lth):
                    vec = r.c(
                        int(self.data[chrom][p]),
                        int(wig2.data[chrom][p]),
                        sum1 - int(self.data[chrom][p]),
                        sum2 - int(wig2.data[chrom][p]),
                    )
                    if self.data[chrom][p] == wig2.data[chrom][p]:
                        out.data[chrom][p] = 1
                    else:
    ...
    def histogram(self, bnum=100000, nonzero_end=False):
        ...
        for cr in self.data:
            counts += numpy.histogram(self.data[cr], bins=bnum, range=(mi, ma))[0]
        return [counts, bins]
    ...
    def load(self, file, gfile, suppress=False):
        ...
        for line in fi:
            if line[0] == "t":
                ...
                sys.stdout.write("Load failure: format not recoganized! " + line + "\n")
    ...
    def loadFixed(self, file, gfile, suppress=False):  # add by Kaifu April 4, 2012
        ...
        if self.step < 1:
            tempf = open(file)
            if not suppress:
                sys.stdout.write("detecting step size ...\n")
            while self.step < 1:
                line = tempf.readline()
                if line == "\n":
                    continue
        ...
        for line in open(file):
            ...
            if line[0] == "t":
                ...
                if not suppress:
                    sys.stdout.write(
                        str(newchr) + " start from position " + str(newstart) + "\n"
                    )  # /instep
                if chrom != "":
                    if chrom not in self.data:
                        self.data[chrom] = numpy.array([0.0])
                    ...
                    if instep != self.step:
                        ...
                        for pos in range(len(lst)):
                            ...
                            if instep < self.step:
                                nplst[functions.div(pos * instep, self.step)] += lst[
                                    pos
                                ]  ########## add by kaifu on Sep 6, 2012 ##########
                            else:
                                for tpos in range(
                                    functions.div(pos * instep, self.step),
                                    functions.div((pos + 1) * instep, self.step),
                                ):
                                    nplst[functions.div(tpos, self.step)] += (
                                        functions.div(
                                            lst[pos] * 1.0 * self.step, instep
                                        )
                                    )  ########## add by kaifu on Sep 6, 2012 ##########
                    self.data[chrom].resize(
                        functions.div(start, self.step) + len(nplst), refcheck=0
                    )
                ...
                try:
                    ...
                    if line[:3] == "nan":
                        ...
                        sys.stdout.write("wrong line: " + str(line[:-1]) + "\n")
                    continue
            ...
            for pos in range(len(lst)):
                ...
                if instep < self.step:
                    ...
                    for tpos in range(
                        ...
                        nplst[functions.div(tpos, self.step)] += functions.div(
                            lst[pos] * 1.0 * self.step, instep
                        )  ########## add by kaifu on Sep 6, 2012 ##########
        self.data[chrom].resize(
            functions.div(start, self.step) + len(nplst), refcheck=0
    ...
    def loadVar(self, file, gfile, suppress=False):
        ...
        for line in fi:
            ...
            if line[0] == "t":
                ...
                if right < 2:
                    sys.stdout.write(
                        "wrong format: " + line + " chrom and span must be provided\n"
                    )
            else:
                col = line.split()
    ...
    def percentile(self, p=[0, 25, 50, 75, 100], bnum=100000, nonzero_end=False):
        ...
        while j < bnum:
            ...
            while functions.div(count * 100.0, total) >= p[i] and i < plth:
                p[i] = functions.div(hs[1][j] * (100 - p[i]), 100.0) + functions.div(
                    hs[1][j + 1] * (p[i]), 100.0
                )
                i += 1
                if i >= plth:
    ...
    def ppois(self, wig2, tchr=""):
        ...
        for chrom in wig1.data:
            ...
            if chrom in wig2.data:
                ...
                while p < lth:
                    if wig1.data[chrom][p] > wig2.data[chrom][p]:
                        if wig1.data[chrom][p] < 1:
                            tstr1[p] = 1
                        else:
                            tstr1[p] = wig1.data[chrom][p]
                        if wig2.data[chrom][p] < 1:
                            tstr2[p] = 1
                        else:
                            tstr2[p] = wig2.data[chrom][p]
                    else:
                        if wig1.data[chrom][p] < 1:
                            tstr2[p] = 1
                        else:
                            tstr2[p] = wig1.data[chrom][p]
                        if wig2.data[chrom][p] < 1:
                            tstr1[p] = 1
                        else:
                            tstr1[p] = wig2.data[chrom][p]
                    p += 1
                ...
                frags = {}
                for end in range(frglen, lth, frglen):
                    frags[end - frglen] = end
                ...
                for start in starts:
                    ...
                    result = functions.ppois(
                        FloatVector(tstr1[start:end]), FloatVector(tstr2[start:end])
                    ) / log(10)
                    p = start
                    while p < end:
                        if tstr1[p] == tstr2[p]:
    ...
    def regionWithinValueRange(self, lowValue=None, highValue=None):
        ...
        for cr in twig.data:
            twig.data[cr][
                numpy.where(
                    [((self.data[cr] < lowValue) | (self.data[cr] > highValue))]
                )[1]
            ] = 0
            twig.data[cr][
                numpy.where(
    ...
    def rvNeg(self):
        ...
        """
        for chrom in self.data:
            functions.div(self.chrSize(chrom), self.step)
            t = self.data[chrom]
    ...
    def save(self, file, format="fixed", step=None, suppress=False):
        ...
        if format == None:  # add by Kaifu on Feb 10, 2014
            ...
            for i in range(total):
                ...
                if val != 0:
                    count += 1
            if functions.div(count * 1.0, total) >= 0.5:
                format = "fixed"
            else:
                format = "var"
        ...
        for chrom in self.data:
            ...
            if format == "fixed":
                ...
                ot = []
                for v in self.data[chrom]:
                    ot.append(str(v))  # str(self.data[chrom][i]))
                outf.write("\n".join(ot) + "\n")
            elif format == "var":
                lth = len(self.data[chrom])
                ...
                i = 0
                while i < lth:
                    if self.data[chrom][i] != 0:
                        outf.write(
                            str(i * step) + "\t" + str(self.data[chrom][i]) + "\n"
                        )
                    i += 1
            elif format == "wiq":
                lth = len(self.data[chrom])
                ...
                sys.stdout.write(
                    ",format not recogonized, will be saved in fixed format, "
                )
                outf.write(
                    "fixedStep chrom="
                    + chrom
                ...
                lth = len(self.data[chrom])
                for i in range(0, lth):
                    outf.write(str(self.data[chrom][i]) + "\n")
        outf.close()
    ...
    def saveChr(self, file, chrom=None, lth=None, format="fixed", step=10):
        ...
        for chrom in self.data:
            ...
            if format == "fixed":
                ...
                if lth == None:
                    lth = len(self.data[chrom]) * self.step
                for i in range(0, lth, step):
                    outf.write(
                        str(self.data[chrom][functions.div(i, self.step)]) + "\n"
                    )
        outf.close()
        sys.stdout.write("completed\n")
    ...
    def sizeAdjust(self, gfile):
        ...
        for chrom in self.data:
            if chrom not in sizes:
                ...
                self.data[chrom].resize(
                    functions.div(sizes[chrom], self.step), refcheck=0
                )
                sys.stdout.write(str(self.data[chrom].size) + "\n")
    ...
    def smooth(self, lmd=100):
        ...
        for chrom in wg1:
            lth = wg1[chrom].size
            if tlmd != 0 - hlmd:
                for p in range(0 - hlmd, tlmd):
                    wg1[chrom][hlmd : (lth - tlmd)] += wg2[chrom][
                        (hlmd + p) : (lth - tlmd + p)
                    ]
            else:
                for p in range(0 - hlmd + 1, tlmd):
                    wg1[chrom][hlmd - 1 : (lth - tlmd)] += wg2[chrom][
                        (hlmd - 1 + p) : (lth - tlmd + p)
                    ]
                for p in [0 - hlmd, tlmd]:
                    wg1[chrom][hlmd : (lth - tlmd)] += (
                        wg2[chrom][(hlmd + p) : (lth - tlmd + p)] * 0.5
                    )
            wg1[chrom][hlmd : (lth - tlmd)] = functions.div(
                wg1[chrom][hlmd : (lth - tlmd)], (lmd) * 1.0
            )
        self.data = wg1
        return True
    ...
    def subtract(self, wig2):
        ...
        for chrom in chrs:
            ...
            lth2 = len(wig2.data[chrom])
            if lth1 > lth2:
                wig2.data[chrom].resize(int(lth1), refcheck=0)
            else:
                self.data[chrom].resize(int(lth2), refcheck=0)
            self.data[chrom] -= wig2.data[chrom]
        wig2.clearEmptyEnd()
        return True
    ...
    def sumWithinValueRange(self, lowValue, highValue):
        ...
        for cr in twig.data:
            ...
            i = 0
            while i < size:
                if twig.data[cr][i] >= lowValue and twig.data[cr][i] <= highValue:
                    total += twig.data[cr][i]
                i += 1
        return total
        ...
        for line in infile:
            ...
            for i in range(len(t2)):
                if t2[i] == "M":
                    ...
                    self.data[chrom][
                        functions.div(start, step) : functions.div(end, step)
                    ] += 1
                elif t2[i] == "D":
                    start = start + int(t1[i])
                elif t2[i] == "N":
                    start = start + int(t1[i])
